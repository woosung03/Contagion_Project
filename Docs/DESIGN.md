...
| `--color-surface-mint` | `rgba(150, 255, 185, 0.04)` | Direction C(Data Wall Terminal) 카드/행 배경 전용 초저알파 민트 틴트 — Surface Ladder(백색 alpha 8단계, 2.7절)와는 별도 축. "회색 카드"를 투명이 아니라 accent-glow 자체의 극히 옅은 틴트로 채워 카드 형태는 유지하면서 회색 인상만 줄인다(2026-07-17, CountryStatusPanel 1차 적용 — `hero-stat-tile`/`status-row`/`continent-header`) |
...
- Hero Stats: 2×2 타일, 타일 폭 48%, 값 폰트 `--font-size-xl`(24px). 배경 `--color-surface-mint`,
  테두리 `--color-accent-glow`(저반복 4개뿐이라 full 강도 — Direction C Color Pass 1).
- 대륙 아코디언: 코너컷 금지(리스트 반복 요소), 배경 `--color-surface-mint`, 좌측바
  `--color-accent-glow`(상시 노출, Direction C Color Pass 1 — 과거 soft+hover 강조 방식은
  터치 기기에서 hover가 거의 안 걸려 폐기) — 대륙 헤더는 "국가 데이터"가 아니라 "UI 크롬"이라
  Severity 4색 사용 금지(2.2절 Don't).
- 48개국 리스트 행(`status-row`): 배경 `--color-surface-mint`(Direction C Color Pass 1, 회색
  대신 초저알파 민트 틴트로 카드 형태 유지), 좌측 4px severity accent-bar(2.2절 4색), radius 0.
...
| 사용 규칙 | 값 색상은 14절 `data-value--*` severity 모디파이어를 그대로 얹어 재사용 가능(신규 색 도입 없음) |
...
| 사용 규칙 | 코너컷 부착 금지(8절 "수십 개 반복 리스트" 규칙), 좌측 accent-bar는 `--color-accent-glow-soft`만 사용 — 대륙 헤더는 "국가 데이터"가 아니라 "UI 크롬"이라 2.2절 severity 4색 사용 금지 |
