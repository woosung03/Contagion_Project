...
| `--color-bg-panel-alt` | `rgba(10, 10, 20, 0.92)` | `upgrade-box`(Bottom Sheet Content Area, 12.2절 — 2026-07-21 이전엔 Fullscreen `upgrade-root` 자체 배경이었으나 재분류로 내부 패널 박스로 이동) |
| `--color-bg-panel` | `rgba(15, 15, 25, 0.95)` | Overlay Panel(ResearchPopup)/Bottom Sheet Compact(CountryPopup 헤더 등) 등 팝업류 |
| `--color-bg-panel-strong` | `rgba(9, 9, 16, 1)` | Bottom Sheet Compact(CountryPopup 본문)/Content Area(`status-box`/`ranking-box`, 12.2절) — 지도가 비쳐 보이면 안 되는 화면, 완전 불투명 |
...
| `--color-bg-scrim-soft` | `rgba(0, 0, 0, 0.55)` | **현재 미사용.** HUD 상단바(news-panel, 舊 event-dock) 전용으로 등재돼 있었으나 실측 결과 `--color-bg-panel`을 쓴다 — HUD 5개 컴포넌트(resource-strip/news-panel/population-bar/graph-panel/action-strip) 전부 동일하게 `--color-bg-panel` 하나로 통일돼 있다(2026-07-17 UI Polish Audit로 정정 — 기존엔 event-dock 하나만 이 상태로 각주 처리됐으나 graph-panel/action-strip도 동일했음) |
...
> **[Validation Audit, 2026-07-23] 값 전면 갱신** — 아래 표는 2026-07-21·22 두 차례의
> "Scale Pass"/"PanelSettings Correction Pass"(참조 해상도 오설정 전제로 커졌던 값을 재조정)
> 이후의 `Theme.uss` 실측값으로 갱신했다. 갱신 전 이 문서는 그 두 차례 조정 중 어느 것도
> 반영하지 않은 상태였다(①구현이 맞고 문서가 낡은 사례) — Typography 9개 토큰 전부가 실제
> 값과 달랐다. usage 열은 실사용처 재확인 결과 대부분 유효해 그대로 유지한다.

| 토큰 | font-size | weight | letter-spacing | usage |
|---|---|---|---|---|
| `--font-size-caption-2xs` | 9px | bold | 0.5px | badge-tag 최소 텍스트 — 실사용처는 `CountryStatusPanel`(`status-row__badges .badge-tag`)이다. CountryPopup의 badge-tag는 실측 결과 font-size가 지정돼 있지 않아(Unity 기본 크기) 이 토큰을 쓰지 않는다 — 최초 등재 시 사용처를 잘못 표기한 오류를 정정. **[Validation Audit] 모바일 최소 가독성 기준(일반 권장 11px 이상) 미달 — TODO, 부록 A 참고, 지금은 값 변경하지 않음** |
| `--font-size-caption-xs` | 10px | bold | 1px | world-status-frame__caption 등 초소형 유틸 라벨 |
| `--font-size-caption` | 11px | bold | 1px(`--tracking-caption`) | tactical-caption, lab-caption |
| `--font-size-xs` | **15px**(구 13px) | bold | 0.5px(`--tracking-label`) | data-label/data-value, 판독 행 기본 크기. Role 별칭 `--font-caption-dense`(트리 노드 등 밀집 요소 전용)와 동일 값 |
| `--font-size-sm` | **17px**(구 14px) | bold | 0.5px | stat-label, popup 값 텍스트 |
| `--font-size-body` | **20px**(구 16px) | regular | 0 | 일반 본문(그래프 라벨 등). Role 별칭 `--font-body`와 동일 값 |
| `--font-size-md` | **23px**(구 18px) | bold | 0 | status-row__name, hero-stat-tile__label. Role 별칭 `--font-subtitle`와 동일 값 |
| `--font-size-lg` | **25px**(구 20px) | bold | 0 | country-row__name, ranking-value — Role 밖 "리스트 항목 이름" 전용 |
| `--font-size-xl` | **32px**(구 24px) | bold | 0 | hero-stat-tile__value, upgrade-header__dna. Role 별칭 `--font-title`와 동일 값 |
| `--font-size-xxl` | **34px**(구 26px) | bold | 0 | Role 별칭 `--font-panel-title`("Bottom Sheet 전체 패널 헤더" 용도로 정의)은 존재하나, 실측 결과 이 토큰을 직접 참조하는 USS 규칙은 현재 0건 — 여전히 예약 상태(사용 전 이 문서 갱신 필수라는 원 주의사항은 유지) |
| `--font-size-display` | **44px**(구 32px) | bold | 0 | mainmenu-title |
| `--font-size-hero` | **52px**(구 40px) | bold | 0 | 엔딩 판정 타이틀 |
...
> **[Validation Audit, 2026-07-23] 값 전면 갱신** — 2026-07-21·22 두 차례의 Scale Pass 이후
> 실측값으로 갱신(①구현이 맞고 문서가 낡은 사례). 갱신 전 이 문서는 6개 토큰 중
> `--space-xs`(4px) 1개만 실제 값과 일치했다.

| 토큰 | 값 |
|---|---|
| `--space-xs` | 4px |
| `--space-sm` | **10px**(구 8px) |
| `--space-md` | **16px**(구 12px) |
| `--space-lg` | **22px**(구 16px) |
| `--space-xl` | **28px**(구 20px) |
| `--space-xxl` | **36px**(구 24px) |

**"4의 배수 그리드"라는 과거 원칙도 이번 갱신으로 폐기한다** — 실측값(10/16/22/28/36)은
4의 배수가 아니다. 지금은 "4px 단위로 단조 증가하는 6단계 스케일"이라는 것만 유지되는
사실이며, 배수 관계 자체는 더 이상 규칙이 아니다.

**Do**: 패딩/마진은 항상 이 6개 값 중 하나를 쓴다. **Don't**: 임의 여백을 만들지 않는다(단,
아이콘/작은 장식 요소의 `2px`/`3px`/`6px` 같은 미세 조정은 예외로 허용한다 — data-row 하단
여백 2px, corner-cut 오프셋 등 이미 실측 확인된 관행).

**[Validation Audit] 알려진 잔여 이탈**: `MainMenu.uss`의 `.country-row`(`padding: 8px 12px;
margin-bottom: 10px;`)가 갱신 전 구 스케일 값 그대로 남아있다(③구현 미수정 — 이번 감사는
문서만 다루므로 코드는 변경하지 않음, 부록 A에 TODO로 별도 기록).
...
### 6.1 Transition & Animation System (신규 등재, Validation Audit 2026-07-23)

> 이 문서는 갱신 전까지 Transition/Animation 관련 규칙이 전혀 없었다(③DESIGN.md 자체가
> 모바일 Design System으로서 부족했던 항목 — 코드에는 이미 실사용 중인 토큰이 있었음).
> 새 애니메이션을 만드는 것이 아니라, 이미 존재하는 것을 문서화만 한다.

| 토큰 | 값 | 용도 |
|---|---|---|
| `--transition-fast` | 0.1s | 버튼 `:active` 등 즉각적 반응이 필요한 상호작용 |
| `--transition-normal` | 0.18s | hover 등 상대적으로 느긋한 상태 전환(터치 기기에선 실효과 낮음, 에디터/PC 확인용) |

**실사용 패턴**: 전 화면 공통 `Button:active { scale: 0.96 0.96; }`(축소만 사용, 확대 없음 —
코너 장식이 눌림 애니메이션 중 부모 경계를 넘어설 위험이 구조적으로 없다는 뜻이기도 하다).
box-shadow 등 UI Toolkit 미지원 프로퍼티에 기반한 애니메이션은 없음(플랫폼 제약과 일치).
...
| 분류 | 정의 | 해당 화면 |
|---|---|---|
| **Fullscreen** | 화면 전체를 차지하는 루트 캔버스. `--color-bg-root` 또는 `--color-bg-panel-alt` | MainMenu, CountrySelect, EndingScreen |
| **Overlay Panel** | 화면 중앙에 뜨는 독립 모달(가장자리 비고정). `TacticalModalController` 상속 | ResearchPopup, ImportantEventPopup |
| **Bottom Sheet** | HUD Content Area 규격(12절)을 따라 Status Bar 아래·Bottom Navigation 위 공간만 차지 | CountryPopup(Compact, 하단 고정+콘텐츠 높이), CountryStatusPanel·RankingPanel·UpgradeTree(Content Area) |
| **HUD Chrome** | Gameplay 화면에 상시 존재하는 크롬(Status Bar/Bottom Navigation) | resource-strip, action-strip |

**[HUD Layout Architecture, 2026-07-21 — Layout Architecture Refactoring]** HUD는 항상
`Status Bar(resource-strip, 고정)` → `Content Area(교체됨)` → `Bottom Navigation(action-strip,
고정)` 3단 구조를 유지한다. **Gameplay/Research(UpgradeTree)/GlobalStatus/Leaderboard 4개
화면 전부 동일한 Content Area 규격을 공유한다** — 어느 화면이 열려 있든 항상 Status Bar
바로 아래부터 Bottom Navigation 바로 위까지만 차지하고, 그 값은 %(해상도/DPI 의존)가 아니라
Theme.uss `--hud-status-bar-height`/`--hud-bottom-nav-height`(Hud의 resource-strip/action-strip
실측 높이) 토큰을 공유하는 순수 Flex 계산으로 나온다. 상세 메커니즘은 12절 참고.

신규 화면을 만들 때 반드시 이 4개 중 하나로 먼저 분류한 뒤 해당 절의 정확한 수치를 적용한다.
분류 없이 임의 레이아웃을 만들지 않는다.

**Information Popup(정보 표시형 팝업) — 위치 분류를 가로지르는 목적 기준 카테고리(신규 등재,
Popup Content Spacing System 2026-07-24)**: 위 4분류는 "화면이 어디에 위치하는가"만 다룬다.
그와 별개로 "사용자가 정보를 읽기 위해 여는 화면인가"라는 **목적 기준**의 카테고리가 있다 —
`ResearchPopup`(Overlay Panel)·`CountryPopup`(Bottom Sheet Compact)·`ImportantEventPopup`
(Overlay Panel)이 여기 해당하며, 위치 분류(Overlay Panel/Bottom Sheet)가 서로 달라도 **내부
콘텐츠 Padding/요소 간 간격은 11.2절(Popup Content Spacing System)이라는 동일한 표를 공유한다.**
신규 Information Popup을 추가할 때도 위치는 7절/12.1절 중 알맞은 것으로 분류하고, 내부 간격은
11.2절을 그대로 적용한다 — 화면마다 별도 간격 규칙을 새로 만들지 않는다.

### 7.1 Overlay Panel 추가 규칙 (Safe Area / Height Overflow / Aspect Ratio, 신규 등재 — Validation Audit 2026-07-23)

> Bottom Sheet(12절)는 Safe Area·높이 규칙이 이미 있었지만, Overlay Panel은 분류만 있고
> 아래 세 규칙이 전부 빠져 있었다(③DESIGN.md 자체의 공백). 실제 배선 확인 결과 ResearchPopup/
> ImportantEventPopup 둘 다 이 공백 그대로 `SafeAreaApplier` 미배선 상태다 — 규칙 신설과
> 별개로 배선 자체는 코드 작업이라 이번 문서 갱신 범위 밖(부록 A에 TODO로 기록).

**범위**: 이 절은 Overlay Panel의 위치/앵커링/Safe Area/Height Overflow만 다룬다. 내부 콘텐츠
Padding·요소 간 간격(Header/Body/Footer 세부 수치)은 이 절이 아니라 11.2절(Popup Content
Spacing System)을 따른다 — 두 절이 같은 내용을 중복 정의하지 않는다.

**Overlay Panel Safe Area 규칙**: 화면 중앙에 뜨는 팝업이라도 상단 Safe Area(노치/펀치홀)를
침범하지 않는다. Overlay Panel은 화면 전체를 차지하지 않으므로 좌우 침범 위험은 낮지만, `top:%`
기준 위치이므로 **상단 인셋만큼은 Bottom Sheet(12절)와 동일한 기준으로 확보**한다.

**Popup Height Overflow 규칙**: Overlay Panel의 콘텐츠 높이가 가용 화면 높이를 초과할 가능성이
있으면 반드시 내부 ScrollView로 감싼다. 판단 기준은 Reference Resolution(480×1040, 19.5:9)이
아니라 **실제 지원 화면비 범위(16:9~21:9) 중 가장 불리한 조건**이다 — PanelSettings이 Match=0
(폭 기준 스케일)이므로 가로는 항상 정확히 비례하지만, 세로 가용 공간은 화면비에 따라
참조 대비 최대 -18%(16:9 기준)까지 줄어든다. `top:30%` 고정 위치 + Foldout 등 콘텐츠 확장
요소가 있는 화면(예: ResearchPopup)은 이 축소분을 반드시 감안한다.

**Overlay ScrollView 규칙**: 12.2절의 Content Area ScrollView 규칙(`flex-grow:1; flex-basis:0`,
세로 오버플로우 방지)을 Overlay Panel에도 동일하게 적용한다 — 지금까지 이 규칙은 Content
Area(Bottom Sheet)에만 적용되고 있었다.

**Aspect Ratio Validation 규칙**: Reference Resolution(480×1040, 19.5:9)만으로 검증을 끝내지
않는다. 신규 화면·레이아웃 변경 시 최소 아래 화면비에서 확인한다 — **16:9를 최우선으로
검증**(세로 가용 공간이 가장 적어 Height Overflow가 가장 먼저 드러나는 조건).

| 화면비 | 참조(1040) 대비 논리적 세로 높이 |
|---|---|
| 16:9 | ≈853 (-18%) — **최우선 검증** |
| 18:9 | ≈960 (-8%) |
| 19.5:9(참조) | 1040 (기준) |
...
| `hero-stat-tile`(신규 등재 — Popup Content Spacing System Audit, 2026-07-24) | 없음(투명) | 1px(`--stroke-hairline`) `--color-accent-glow` | 0 | KPI 2×2 큰 숫자 타일. `width:48%; padding: --space-sm --space-md; margin-bottom: --space-sm`, `__label`(`--font-size-xs`/`--color-text-tertiary`)·`__value`(`--font-size-xl`/bold/`--color-text-primary`, `margin-top:2px` 하드코딩 잔존) 구조. **`CountryStatusPanel.uss`(`hero-stat-tile`)와 `CountryPopup.uss`(`popup-hero-stat-cell`) 두 파일에 완전히 동일한 값으로 중복 구현돼 있다** — 두 화면이 스타일시트를 공유하지 않는 구조라 로컬 복제됐던 것을, 값 자체를 이 표로 정본화해 향후 드리프트(둘 중 하나만 수정되는 사고)를 방지한다. 클래스명 통합은 이번 범위 밖(USS 미변경) |
...
**소비처**: `RankingPanel.uss`(확인/닫기) — Information Popup이 아니므로 `Tactical.uss` 정본
(`margin-top: --space-sm`)을 그대로 사용한다. Information Popup 3종(`ResearchPopup.uss`/
`CountryPopup.uss`/`ImportantEventPopup.uss`)은 버튼 스타일(`popup-footer-button`/`--confirm`)은
정본을 그대로 쓰되, `modal-footer`의 `margin-top`은 11.2절 Popup Content Spacing System 값
(`--space-lg`)을 화면 로컬로 override하는 것이 규칙이다(`#modal-footer` name 선택자로
`Tactical.uss`는 건드리지 않는다) — **단, 이 문서 갱신 시점 기준 실제 코드는 아직 전부
일치하지 않는다**: `ResearchPopup.uss`만 22px(`--space-lg`)로 갱신됐고, `CountryPopup.uss`는
16px(`--space-md`), `ImportantEventPopup.uss`는 override 자체가 없어 기본값 10px 그대로다 —
코드 정렬은 부록 A TODO로 기록(이번 작업은 문서만 갱신). **ResearchPopup의 확정 버튼 텍스트는
...
### 11.2 Popup Content Spacing System (Information Popup 공통 — 신규 일반화 2026-07-24)

> **연혁**: 이 절은 원래 "Node Detail Popup(ResearchPopup) Content Spacing"이라는 ResearchPopup
> 전용 규칙이었다(Layout Audit 2026-07-24). 이후 Country Information Popup·News Popup을
> 전수 조사한 결과 세 화면 모두 같은 문제(Overlay Panel(7절)/Bottom Sheet Compact(12.1절)에는
> 위치·Safe Area 규칙만 있고 **내부 콘텐츠 Padding·요소 간 간격 자체가 정의돼 있지 않음**)를
> 공유하고 있음이 확인돼, 이 절을 **Information Popup(7절 정의) 공통 규칙**으로 일반화했다.
> ResearchPopup은 더 이상 "이 규칙의 유일한 대상"이 아니라 "이 규칙을 쓰는 세 사례 중 하나"다.

**적용 대상**: `ResearchPopup`(Overlay Panel)·`CountryPopup`(Bottom Sheet Compact)·
`ImportantEventPopup`(Overlay Panel) — 그리고 향후 추가되는 모든 Information Popup. 위치
분류(Overlay Panel인지 Bottom Sheet Compact인지)는 7절/12.1절이 결정하고, **이 절은 그 위치
분류와 무관하게 내부 콘텐츠 간격만 규정한다.**

**두 축 — Vertical Rhythm과 Horizontal Layout(신규 등재, 2026-07-24)**: 이 절은 서로 독립된
두 축을 함께 규정한다. **Vertical Rhythm(세로 리듬)**은 요소들이 위/아래로 "얼마나 떨어져
있는가"(`margin-top`/`margin-bottom`, `padding-top`/`padding-bottom`)를 다루고 — 아래 첫 번째
표가 이를 규정한다. **Horizontal Layout(가로 정렬, Content Safe Area)**은 모든 콘텐츠가 좌/우로
"어느 기준선에서 시작하는가"(`margin-left`/`margin-right`, `padding-left`/`padding-right`)를
다루며 — 이 절 후반의 별도 하위 규칙이 이를 규정한다. 두 축은 서로 다른 CSS 속성을 다루므로
겹치거나 충돌하지 않는다: Vertical 표의 어떤 행도 좌우 값을 건드리지 않고, Horizontal 규칙의
어떤 항목도 상하 값을 건드리지 않는다.

**Vertical Rhythm — 세로 간격 값**

| 요소 | 값 | Design Rationale |
|---|---|---|
| **Content Padding**(Popup 내부 Padding, 상하좌우 동일) | `--space-xl`(28px) | 권장 여백 범위(24~32px) 충족 — 좁으면 콘텐츠가 코너컷/모서리에 바짝 붙어 답답해 보인다 |
| **Header Bottom Padding** | `--space-sm`(10px) | 제목 텍스트와 헤더 하단 구분선 사이 여백 — 공용 `tactical-panel__header`(8절) 기본값 3px는 텍스트가 구분선에 붙어 보일 만큼 좁다 |
| **Header Bottom Margin** | `--space-md`(16px) | 헤더(제목)와 본문을 시각적으로 분리한다 — 공용 기본값(`--space-xs` 4px)로는 제목이 "본문의 첫 줄"처럼 붙어 보인다 |
| **Close Button / Corner Safe Area** | 신규 값 없음 — 8절 `--space-corner-safe`(14px)를 그대로 재사용 | 코너컷 그래픽과 버튼이 겹치지 않는 여백은 이미 8절에 있다. Content Padding(28px)이 커지면서 모서리와의 절대 거리는 자연히 함께 늘어나므로 별도 값을 만들 필요가 없다 |
| **Header 내부 배치 기준** | `flex-direction: row; justify-content: space-between; align-items: center` | 제목과 보조 요소(닫기 버튼, 국기, 배지 등)를 한 줄에 정렬하는 공통 골격 — 화면마다 국기·부제 같은 추가 요소가 있어도 이 골격만 확장하고 새 배치 패턴을 도입하지 않는다 |
| **Header → 첫 요소** | 신규 값 없음 — Header Bottom Margin(16px)이 곧 이 간격이다 | 헤더 아래 첫 본문 요소(설명/통계 그리드)에 별도 `margin-top`을 추가하면 Header Bottom Margin과 이중으로 겹쳐 간격이 과도해진다 — 헤더 쪽 margin 하나로만 만든다 |
| **Description → 다음 요소**(예: Cost) | `margin-bottom: --space-lg`(22px) | 설명(맥락 정보)과 수치 정보(비용 등) 사이의 위계 전환 신호 — Section 간 간격(16px)보다 한 단계 더 넓게 잡아 "다른 종류의 정보로 넘어간다"는 것을 보여준다 |
| **Section 제목 → 내용 간격** | `--space-sm`(10px) | 제목과 그 아래 나열 항목이 한 그룹임을 보이되, 제목이 목록 항목처럼 붙어 보이지 않을 최소 분리 |
| **Section → Section 간격** | `--space-md`(16px) | 서로 다른 두 섹션(예: 효과/주의)이 뭉쳐 보이지 않게 하되, Description→다음 요소(22px)보다는 좁게 잡아 "같은 성격의 정보 항목끼리는 더 가깝다"는 위계를 만든다 |
| **독립 정보 블록**(Cost 등) 하단 여백 | `margin-bottom: --space-md`(16px) | Cost는 Section이 아니라 "그 자체로 완결된 수치 정보"라 Section 간 간격과 동일한 값으로 다음 Section과 대등하게 분리한다 — 위(Description) 쪽 22px보다 아래쪽을 좁게 잡아 "위에서 내려온 정보의 결론"이라는 인상을 준다 |
| **Label / Value 간격** | 신규 값 없음 — 14절 `data-row` 정본(`padding-bottom:2px; margin-bottom:2px`) 그대로 재사용 | 수치 판독 행은 이미 14절에 정의된 가장 기본 단위 — 이 절에서 다시 정의하지 않는다 |
| **Bullet 간격** | `--space-xs`(4px) | 같은 섹션 내 나열 항목이므로 토큰 중 가장 작은 값만 사용 — 이보다 넓으면 서로 다른 섹션처럼 보인다 |
| **Footer top margin**(본문 → Footer) | `--space-lg`(22px) | Footer는 "정보를 다 읽은 뒤 취하는 액션" 영역이므로 본문과 명확히 분리해, 정보를 훑는 도중 실수로 액션 버튼을 누르지 않도록 시각적 완충 지대를 만든다. 11.1절 공용 기본값(`--space-sm` 10px)은 RankingPanel처럼 액션 중심 화면 기준이라 Information Popup에는 좁다 |
| **Popup 최소 크기** | 정의하지 않음(의도적) | 폭은 화면마다 콘텐츠 폭에 맞춰 고정값으로 결정(예: ResearchPopup 340px, ImportantEventPopup 320px, CountryPopup은 Bottom Sheet라 100%), 높이는 콘텐츠 기반 + `max-height`(7.1절 Height Overflow 규칙/12.1절 68%)로 상한만 둔다 — Information Popup은 화면마다 정보량이 달라 최소 높이를 강제할 이유가 없다 |

**현재 적용 현황(문서 vs 실제 USS, 2026-07-24 기준)**:

| 항목 | ResearchPopup | CountryPopup | ImportantEventPopup |
|---|---|---|---|
| Content Padding(28px) | ✅ 반영됨 | ✅ 반영됨(2026-07-24 USS 정렬) | ❌ 22px(`--space-lg`) 그대로 |
| Header Bottom Padding/Margin(10px/16px) | ✅ 반영됨 | ✅ 반영됨(2026-07-24 USS 정렬) | ❌ 로컬 override 없음(공용 3px/4px 그대로) |
| Footer top margin(22px) | ✅ 반영됨 | ✅ 반영됨(2026-07-24 USS 정렬) | ❌ override 없음(공용 10px) |
| Section/Bullet 간격 | ✅ 반영됨(토큰화 완료) | N/A(섹션/불릿 구조 없음) | N/A(섹션/불릿 구조 없음) |

이 표는 **문서(Source of Truth) 갱신 결과일 뿐 코드 변경이 아니다** — ImportantEventPopup의
실제 USS 정렬은 부록 A에 TODO로 기록하고 별도 세션에서 진행한다.

**Do**: 위 값은 전부 4절 Spacing 토큰(`--space-xs`~`--space-xl`)만 사용한다 — 새 spacing 값을
도입하지 않는다. 신규 Information Popup은 이 표를 그대로 적용하고, 위치(Overlay Panel/Bottom
Sheet Compact)만 7절/12.1절에서 분류한다.
**Don't**: 공용 `tactical-panel__header`/`modal-footer`(`Tactical.uss`) 자체의 기본값을 바꾸지
않는다 — 다른 화면(CountryStatusPanel/UpgradeTree/RankingPanel/MainMenu/CountrySelect/
EndingScreen)이 전부 그 값을 공유하므로, 화면별 로컬 클래스(`.popup-header`, `#modal-footer`
등)에서만 override한다. Information Popup이 아닌 화면(RankingPanel 등)에 이 표의 값을
강제하지 않는다.

**Horizontal Layout — Content Safe Area (신규 등재, 2026-07-24)**

> **원인 진단**: Research/Country/News 세 화면 모두 실제 인게임에서 "콘텐츠가 화면 가장자리에
> 가깝다"는 인상을 준다. 원인은 USS 수치 부족이 아니다 — 이 문서는 지금까지
> `Popup Border → Popup Padding → Content`까지만 규정했을 뿐, **"Padding 안쪽에서 모든 콘텐츠가
> 동일한 좌우 기준선을 지켜야 한다"는 규율(Content Safe Area) 자체가 어디에도 없었다.** 즉
> Header/Section/Footer가 각자 다른 좌우 여백을 줘도 이를 막는 문서 규칙이 존재하지 않았다는
> 뜻이다 — 이 항목은 그 공백을 메운다.
...
> 이 다이어그램은 수치가 아니라 **레이아웃 철학**을 보여주는 용도다 — 실제 폭 값은 항상 아래
> 표의 Content Padding(Vertical Rhythm 표와 동일한 값)을 그대로 쓴다.

| 원칙 | 규칙 | Design Rationale |
|---|---|---|
| **Content Safe Area** | Popup Border에 콘텐츠를 직접 붙이지 않는다. Content Safe Area의 좌우 폭은 위 Vertical Rhythm 표의 **Content Padding**(`--space-xl`, 28px)과 동일한 값을 그대로 좌우 기준선으로 쓴다 — Horizontal 전용 신규 토큰을 만들지 않는다 | Padding 값 자체는 이미 있었지만 "이 안쪽이 모든 콘텐츠가 지켜야 할 안전 영역"이라는 개념이 문서화되지 않아, 화면마다 이 경계를 실제로 지키는지 검증할 기준이 없었다 |
| **Horizontal Alignment** | Header·Description·Hero Stat·KPI Card·Data Row·Text Block·Section·Footer·Action Button은 전부 Content Safe Area의 동일한 좌우 기준선에서 시작한다 — 요소마다, 화면마다 별도의 좌우 들여쓰기(추가 `margin-left`/`margin-right`, `padding-left`/`padding-right`)를 주지 않는다 | 요소마다 좌우 시작점이 다르면 같은 팝업 안에서도 "정렬이 안 맞다"는 인상을 준다 — Vertical Rhythm이 아무리 정돈돼 있어도 Horizontal이 어긋나면 답답함/산만함이 그대로 남는다 |
| **Visual Balance** | 사용자는 Popup Border보다 콘텐츠를 먼저 인식해야 한다 — 콘텐츠가 화면 가장자리에 붙어 보이지 않을 만큼 충분한 Horizontal 여백을 확보한다 | 테두리는 콘텐츠를 담는 그릇일 뿐이다. 테두리가 먼저 눈에 들어오면 "무엇을 보여주는 화면인지"보다 "이 화면이 어디까지인지"가 먼저 읽혀 정보 전달이 늦어진다 |
| **Consistency** | 같은 Popup 안에서 Section마다 좌우 시작점이 달라지지 않는다 — 모든 정보 블록은 같은 기준선에서 시작한다 | Vertical Rhythm이 "위아래 리듬"을 맞추듯 Horizontal도 "좌우 리듬"을 맞춰야 화면 전체가 하나의 시스템으로 보인다 |
| **Mobile Readability** | 모바일에서는 텍스트/데이터가 화면 물리적 가장자리에서 충분히 떨어져 있어야 한다 — 가독성을 Padding 최소화보다 항상 우선한다 | 세로 화면(Portrait 고정, CLAUDE.md 제약)은 좌우 폭 자체가 좁아 여백 부족이 체감되는 정도가 세로 여백 부족보다 훨씬 크다 |

**Bottom Sheet Compact는 Content Safe Area가 유일한 완충 지대다**: Overlay Panel
(`ResearchPopup` 340px/`ImportantEventPopup` 320px)은 참조 해상도(480×1040, 7.1절) 대비 폭이
좁아 화면 가장자리와 팝업 테두리 사이에 이미 자체 여백(약 70~80px)이 있고, 그 안쪽에 Content
Padding(28px)이 한 번 더 있다. 반면 Bottom Sheet Compact(`CountryPopup`, `width:100%;
left:0; right:0`, 12.1절)는 팝업 테두리 자체가 화면 물리적 가장자리와 맞닿아 있어, 콘텐츠와
화면 가장자리 사이의 유일한 완충 지대가 Content Padding뿐이다 — 그래서 Content Safe Area
규율은 Overlay Panel보다 **Bottom Sheet Compact 계열에서 더 결정적**이다.

**Do**: Content Safe Area의 좌우 폭은 항상 Vertical Rhythm 표의 Content Padding 값을 그대로
재사용한다 — Horizontal 전용 토큰을 새로 만들지 않는다. 신규 Information Popup은 모든 콘텐츠
요소를 이 기준선에 맞춰 배치한다.
**Don't**: 특정 섹션/요소에만 추가 좌우 `margin`/`padding`을 줘서 그 요소만 안쪽으로 들여쓰거나
바깥쪽으로 밀어내지 않는다 — Header에 국기·배지처럼 화면 고유 요소가 있어도(12.1절 CountryPopup
등) 그 요소들은 Content Safe Area 안에서 배치되고, Safe Area 경계 자체를 넓히거나 좁히지
않는다.
...
Bottom Sheet는 두 변형(Compact/Content Area)으로 표준화한다. 신규 화면은 반드시 이 중 하나를
선택한다 — 임의의 top/bottom % 값을 새로 정하지 않는다(Content Area는 12.2절 — % 대신
`--hud-status-bar-height`/`--hud-bottom-nav-height` 토큰 기반 spacer로 계산).

### 12.1 Compact (예: CountryPopup)

> **범위**: 이 절은 Bottom Sheet Compact의 위치/앵커링/높이 상한/Safe Area만 다룬다. 내부
> 콘텐츠 Padding·Header/Footer 간격 세부 수치는 11.2절(Popup Content Spacing System)을
> 따른다 — CountryPopup은 그 표의 "Bottom Sheet Compact 적용 사례"다.

| 항목 | 규격 |
|---|---|
| **Anchoring** | `position: absolute; left:0; right:0; bottom:0` — `top` 지정하지 않음(콘텐츠 높이 기반) |
| **Width** | 100% |
| **Height** | 콘텐츠 기반, **`max-height: 68%`** — [Doc Sync, 2026-07-24] 실제 구현은 2026-07-22 UI Visual Calibration Pass에서 이미 60%→68%로 상향돼 있었으나(hero-stat-tile 2행 + data-row 7행 구성의 실제 콘텐츠 높이 반영, `CountryPopup.uss` 주석 참고) 이 문서는 60%로 낡아 있었다 — **문서-구현 불일치를 확인해 구현값(68%)에 맞춰 문서를 갱신**(0절 원칙: 화면별 `.uss` 값이 실제로 변경된 경우는 문서가 그 실측을 따라간다) |
| **Safe Area** | 하단 `padding-bottom`에 모바일 제스처 바 인셋 반영 |
| **Header 구성 요소** | 국기/타이틀/부제·배지/닫기(32px) 가로 배치 — 이 조합은 CountryPopup 고유(다른 Information Popup엔 국기/부제가 없음). 내부 padding/margin 수치는 11.2절 따름 |
| **Content/Footer 간격** | 11.2절(Popup Content Spacing System) 따름 — 내부 그리드 자체(quick-stat-grid 등)는 13/14절 컴포넌트 사용 |
| 배경 | `--color-bg-panel-strong` |
| radius | 0 |

### 12.2 Content Area (예: CountryStatus, Ranking, UpgradeTree — 2026-07-21 Layout Architecture Refactoring로 % 프리셋 폐기)

**배경**: 기존엔 `position:absolute; top:3%/42%; bottom:12.5%`처럼 화면 전체에 대한 퍼센트로
직접 자기 크기를 잡았다 — 이 값들은 특정 해상도/DPI에서 눈대중으로 튜닝한 근사치라, 다른
화면비·DPI(특히 PanelSettings가 Constant Physical Size라 기기 DPI에 따라 논리 해상도 자체가
변함, 5절 부록 참고)에서는 Hud의 실제 resource-strip/action-strip 높이와 어긋나 틈이나 겹침이
생길 수 있었다(Responsive Layout Audit, 2026-07-21에서 확인). **이 프리셋(Dense/Compact,
top:3%/42%)은 완전히 폐기됐다 — 새 화면에서 재사용 금지.**

**새 규격 — Content Area 패턴**: CountryStatus/Ranking/UpgradeTree는 Gameplay와 별도의
UIDocument이지만, 전부 아래 3단 구조를 반복한다(각 파일 uxml 참고):

| 요소 | 규격 |
|---|---|
| **root**(`status-root`/`ranking-root`/`upgrade-root`) | 배경/테두리/패딩 없는 순수 `flex-direction:column` 컨테이너. `SafeAreaApplier(targetClass=root 클래스명)`가 여기에 상하 padding을 적용 — hud-root와 동일한 방식(아래 Safe Area 항목 참고) |
| **top spacer**(`content-area-top-spacer`, Tactical.uss 공용) | `height: var(--hud-status-bar-height)`(Theme.uss, resource-strip 실측 높이와 반드시 동일) — 투명, `picking-mode:Ignore`. Hud의 실제 resource-strip이 이 자리에서 그대로 비쳐 보인다 |
| **패널 박스**(`status-box`/`ranking-box`/`upgrade-box`, `tactical-panel`) | `flex-grow:1; flex-basis:0;` — top/bottom spacer 사이 남는 공간만 정확히 차지(§12.2 ScrollView 규칙과 동일한 flex-basis:0 원칙을 컨테이너 자신에도 적용). corner-cut 4개 + 배경(`--color-bg-panel-strong`, 완전 불투명) + `overflow:hidden`은 여기서 그대로 유지 |
| **bottom spacer**(`content-area-bottom-spacer`, Tactical.uss 공용) | `height: var(--hud-bottom-nav-height)`(Theme.uss, action-strip 실측 높이와 반드시 동일) — 투명, `picking-mode:Ignore`. Hud의 실제 action-strip이 이 자리에서 그대로 비쳐 보이고 항상 클릭 가능 |
| **Safe Area** | 3개 화면(Upgrade/CountryStatus/Ranking) 전부 자기 root GameObject에 `SafeAreaApplier`가 붙어 동일하게 적용된다(2026-07-21 이전엔 Upgrade만 있고 CountryStatus/Ranking엔 아예 없었음 — Audit에서 발견, 씬 배선은 `Docs/Archive/unity-editor-task.md` 12절 참고) |
| **Content**(패널 박스 내부) | `ScrollView`(`flex-grow:1; flex-basis:0`, 상위 컨테이너 `overflow:hidden` 필수 — 20절 참고) |
| **Header** | `tactical-panel__header` 가로 배치(타이틀 + 닫기 28px) |
| **Footer** | 없음(콘텐츠 내부 버튼으로 대체) |
| radius | 0 |

**Do**: 새 Bottom Sheet Content Area 화면은 위 3단 구조(top spacer/패널 박스/bottom spacer)를
그대로 복제한다 — `--hud-status-bar-height`/`--hud-bottom-nav-height` 토큰만 참조하면 되고,
새 % 값을 만들 필요가 없다.
**Don't**: `position:absolute`+`top`/`bottom` % 조합을 새로 쓰지 않는다. `--hud-status-bar-height`/
`--hud-bottom-nav-height` 외의 매직 넘버로 spacer 높이를 하드코딩하지 않는다.
...
### 14.1 Number Formatting Policy (Summary / Detail Role 기준, 2026-07-22 신규 등재)

숫자 표기 방식은 값의 종류(정적/동적)가 아니라 **그 숫자를 보여주는 컴포넌트의 역할(Role)**로
결정한다 — 새 화면·필드가 추가될 때마다 "이 값이 계속 자라는가"를 개별 판단할 필요가 없도록,
구조적(위치 기반) 기준 하나로 고정한다.

| Role | 정의 | 표기 | 소비처 |
|---|---|---|---|
| **Summary** | 5초 스캔용 요약 컴포넌트(Hero Stat류) | 만/억/조 축약(`NumberFormatter.FormatSummary()`) | 21.4 Hero Stat Tile, 21.5 CountryPopup Hero Tier, 21.11 EndingScreen Result Summary |
| **Detail** | 정밀 조회용 컴포넌트(Data Row류) | 전체 숫자(`NumberFormatter.FormatDetail()`, `N0`와 동일) | 본절(14절) Data Row 전체, 21.5 CountryPopup Secondary Tier, 랭킹/48개국 목록 |

**축약 규칙**(`Assets/Scripts/UI/NumberFormatter.cs`): 0~9,999는 그대로 / 1만~99.9만 미만은 소수
1자리(예: `172,441` → `17.2만`) / 100만~9999만은 정수(예: `24,531,298` → `2453만`) / 1억 이상은
소수 1자리(예: `1,425,775,850` → `14.3억`) / 1조 이상은 소수 1자리(현재 게임 규모상 사실상
도달하지 않음). decimal 연산을 사용하며, 만/억 경계에서 반올림으로 "10000만"처럼 어색해지는
경우 상위 단위로 자동 승격한다.

**CountryPopup 특이 사례**: DevLog Step 76/77 당시 CountryPopup은 화면 전체가 Detail이라 인구
필드도 전체 숫자를 유지하기로 결정했었다. 이후 CountryPopup 내부가 Hero Tier/Secondary Tier로
역할이 분리되면서(21.5절), 그 결정은 **폐기가 아니라 Role 단위로 세분화**됐다 — Hero Tier에
있는 인구는 이제 Summary Role을 따라 축약 표기하고, Secondary Tier의 나머지 필드는 Step 76/77의
정밀 표기 원칙을 그대로 유지한다.

**컴포넌트 클래스와 Role이 일치하지 않는 예외**: EndingScreen의 결과 요약(21.11절)은 시각적으로
`data-row`/`data-value`(위 표 기준 Detail 컴포넌트)를 그대로 쓰지만, Role은 Summary다 — CSS
클래스명만 보고 "왜 정밀 표기가 아니지?"로 오해하지 않는다.
...
| 컴포넌트 | 높이/폭 | 배경 | 테두리 |
|---|---|---|---|
| `resource-strip`(Status Bar) | `--hud-status-bar-height`(34px, Theme.uss 토큰) | `--color-bg-panel` | 좌우하단 1px `--color-accent-glow-soft` |
| `news-panel` | 전폭, 헤더+`news-scroll`(132px) | `--color-bg-panel` | 상하좌우 1px `--color-accent-glow-soft` |
| `population-bar__track` | 높이 14px | `--color-surface-soft` | 1px, radius `--radius-sm` |
| `graph-panel` 프레임 | 폭 100%(부모 `stat-graph-item`의 `flex-grow:1;flex-basis:0` 3등분 폭 그대로 채움) × 높이 52px 고정 | `--color-surface-soft` | 1px, radius `--radius-sm` |
| `action-strip`(Bottom Navigation) 버튼 | `--touch-target-min`(48px), 전체 높이는 `--hud-bottom-nav-height`(70px, Theme.uss 토큰)와 일치해야 함 | 9절 Button System 그대로 |

`--hud-status-bar-height`/`--hud-bottom-nav-height`(Theme.uss)는 Bottom Sheet Content Area(12.2절)의
top/bottom spacer가 그대로 참조하는 단일 소스다 — `resource-strip`의 `height`나 `action-strip`의
padding/버튼 높이가 바뀌면 이 두 토큰도 반드시 같이 갱신해야 한다.

3세그먼트 비율형 데이터(population-bar)는 항상 healthy→infected→dead 고정 순서,
`flex-basis:0` + `flex-grow` 비율로만 표현(절대 픽셀 계산 금지). 추세형(graph-panel)도
[Gameplay HUD Responsive Refactoring, 2026-07-21] `stat-graph-item` 3개 전부 `flex-grow:1;
flex-basis:0`(고정 폭 190px 제거 — 기존엔 3개 합계 약 594px로 480px대 참조 해상도에서
Scale Mode와 무관하게 항상 넘쳤다) + 프레임 폭 100%(높이만 52px 고정) + 기준선 1개
(`--color-accent-glow-soft`) + 보조 격자 2개(25%/75%, `--color-grid-line`)로 구성한다.
새로 고정폭 스탯 그룹을 만들 때도 이 패턴(각 항목 `flex-grow:1;flex-basis:0`)을 기본으로
쓴다 — 절대 px 폭 지정은 지양.
...
- 분류: Bottom Sheet — Content Area(12.2절, 2026-07-21 Layout Architecture Refactoring로
  `top:3%` % 프리셋 폐기).
- Hero Stats: 2×2 타일, 타일 폭 48%, 값 폰트 `--font-size-xl`(24px). 배경 없음(투명), 테두리
  `--color-accent-glow`(저반복 4개뿐이라 full 강도). 감염자/사망자 값은 Summary Role이라
  `NumberFormatter.FormatSummary()`로 만/억 축약 표기한다(14.1절). 치료제%/소멸국가 수(0~48)는
  값 범위가 항상 작아 축약 여부와 무관하게 결과가 같다.
...
- 분류: Bottom Sheet — Content Area(12.2절). 2026-07-21 HUD Full Screen Panel 개편으로
  Fullscreen에서 재분류됐고(같은 날 Layout Architecture Refactoring로 `top:3%` % 프리셋도
  폐기, 지금은 spacer+flex-grow 구조) — `--color-bg-panel-alt` 배경은 유지(이제 `upgrade-box`에
  적용됨, `upgrade-root` 자체는 배경 없는 순수 컨테이너).
...
| `Hud.uss` | resource-strip/news-panel/population-bar/graph-panel/action-strip | 15절 소비처. 로컬 중복 정의 없음(corner-cut은 Tactical.uss 참조로 정리 완료, UI Audit 1차) |
...
| `CountryStatusPanel.uss` | Bottom Sheet(Content Area) | 12.2/16절 소비처. `.status-row .data-row`(더 이상 어떤 자식도 매치하지 않는 죽은 규칙 — 감염률/사망률이 `status-row__stats-summary` 단일 Label로 교체됨) 정리 대상, 코드 변경 필요(부록 A 참고). `--color-surface-mint`(2.5절)는 이 파일이 아니라 어떤 `.uss`에도 참조가 없다 — 2026-07-17 UI Polish Audit로 "이 파일의 잔재" 서술 정정 |
| `RankingPanel.uss` | Bottom Sheet(Content Area) | 12.2절 소비처. 로컬 중복 정의 없음(modal-footer/popup-footer-button은 Tactical.uss 참조로 정리 완료, UI Audit 1차) |
| `UpgradeTree.uss` | Bottom Sheet(Content Area), 17절 | 12.2/17절 소비처. 2026-07-21 Fullscreen에서 재분류 |
...
- 버튼은 항상 9절 표의 `background-color`/`border`/`border-radius`를 전부 명시적으로
  지정한다.
- Bottom Sheet는 12절의 두 변형(Compact/Content Area) 중 하나만 쓴다.
...
- **Surface 계층/오버플로우 위반**: (2026-07-21 이전 `status-root`, 지금은 `status-box`)에
  `flex-direction`/`overflow`를 명시하지 않아 콘텐츠가 박스 경계 밖으로 흘러넘쳐 배경이
  비치는(지도가 보이는) 버그가 실제 발생 — Bottom Sheet Content Area의 패널 박스(12.2절)는
  항상 `overflow:hidden` + `flex-direction:column` 명시(16절). `UpgradeTree.uss`의 `node-scroll`은
  `flex-basis:0` 누락으로 같은 계열 위험이 있었다가 Layout Architecture Refactoring에서 수정.
...
| 숫자 표기 | Summary Role — INFECTED/DEATHS는 `NumberFormatter.FormatSummary()`로 만/억 축약(14.1절). CURE(%)/EXTINCT(0~48)는 원래 짧아 축약 여부와 무관하게 결과가 같다 |

### 21.5 CountryPopup Body — Hero Tier / Secondary Tier (2026-07-22 갱신, UI Visual Calibration Pass)

> 이전 판(단일 `quick-stat-grid` + `quick-stat-cell` 2열 균등 그리드, 11필드 동일 비중 나열)은
> "스프레드시트처럼 보인다"는 피드백으로 아래 2단 위계로 대체되어 더 이상 쓰이지 않는다.

| 속성 | 값 |
|---|---|
| 목적 | 국가 핵심 지표(11필드)를 2단 위계로 배치 — Hero Tier(4개, 5초 스캔용 큰 숫자) + Secondary Tier(7개, 판독행 목록) |
| 사용 화면 | CountryPopup(Bottom Sheet Compact) |
| Hero Tier(Summary Role) | `quick-stat-grid` > `popup-hero-stat-row`(2행) > `popup-hero-stat-cell`(폭 48%) > `__label` + `__value`. 필드: 인구/감염자/사망자/감염률. 인구·감염자·사망자는 `NumberFormatter.FormatSummary()`로 축약(14.1절), 감염률(%)은 원래 짧아 축약 여부와 무관 |
| Secondary Tier(Detail Role) | `secondary-stat-list` > 14절 `data-row`/`data-label`/`data-value` 그대로 재사용. 필드: 사망률/의료수준/의료부하/세계순위/공항/항만/국경 — 전부 `NumberFormatter.FormatDetail()`(=`N0`) 또는 배지 텍스트, 축약 없음 |
| 사용 규칙 | "등급"성 필드(공항/항만/의료/국경 상태)는 `data-value`에 13절 `badge-tag`를 추가 클래스로 결합한다 |
...
### 21.11 EndingScreen Result Summary (신규 등재, Number Formatting Policy v2)

| 속성 | 값 |
|---|---|
| 목적 | 게임 결과(감염자/사망자 등)를 빠르게 전달하는 Summary Role 컴포넌트 |
| 사용 화면 | EndingScreen |
| 구성 요소 | `stats-rows` > 14절 `data-row`/`data-label`/`data-value` 재사용(신규 CSS 없음) |
| 사용 규칙 | "전 세계 감염자"/"전 세계 사망자"는 컴포넌트 클래스가 `data-row`/`data-value`(14절 기준으로는 Detail)이지만 **Role은 Summary**라 `NumberFormatter.FormatSummary()`로 축약 표기한다(14.1절 "컴포넌트 클래스와 Role이 일치하지 않는 예외" 참고). "붕괴 국가 수"/"경과일"은 값 범위가 항상 작아(0~48, 0~수백) 축약 여부와 무관하게 결과가 같아 `FormatDetail()` 그대로 유지한다 |
...
- **[Validation Audit, 2026-07-23] 헤더 닫기 버튼 최소 크기(TODO / Future Review)** — 9절이
  32×32px를 공식 예외로 규정하지만, 일반 모바일 터치 타겟 권장 최소치(Apple HIG 44pt, Material
  Design 48dp 등)에는 못 미친다. 값 변경은 실기기 터치 QA 이후 결정 — 지금은 문제 인지만
  기록하고 9절 규칙 자체는 변경하지 않는다.
- **[Validation Audit, 2026-07-23] Badge 최소 Font Size(TODO / Future Review)** — 3절
  `--font-size-caption-2xs`(9px)가 실제 정보 텍스트(공항/항구 상태 등)에 쓰이는데, 일반 모바일
  가독성 권장 하한(11px 이상)에 못 미친다. 실기기 QA 이후 상향 여부 결정.
- **[Validation Audit, 2026-07-23] MainMenu `.country-row` Spacing 잔여 이탈(TODO)** — 4절
  갱신에서 확인된 대로 `padding: 8px 12px; margin-bottom: 10px;`가 폐기된 구 스케일 값 그대로
  남아있다. 코드 수정은 이번 문서 갱신 범위 밖.
- **[Validation Audit, 2026-07-23] Overlay Panel Safe Area 미배선(TODO)** — 7.1절 신규 규칙
  기준으로 ResearchPopup/ImportantEventPopup 둘 다 `SafeAreaApplier`가 배선되어 있지 않다.
  규칙은 이번에 신설했으나 실제 배선은 코드 작업이라 범위 밖 — 별도 세션에서 처리 필요.
- **[Popup Content Spacing System, 2026-07-24] CountryPopup/ImportantEventPopup USS 미정렬
  (TODO)** — 11.2절을 ResearchPopup 전용 규칙에서 Information Popup 공통 규칙으로 일반화했으나,
  `CountryPopup.uss`/`ImportantEventPopup.uss`는 아직 이 표의 값(Content Padding 28px, Header
  Bottom Padding/Margin 10px/16px, Footer top margin 22px 등)으로 갱신되지 않았다(11.2절
  "현재 적용 현황" 표 참고). 이번 세션은 문서만 갱신했고, 두 화면의 USS를 이 표에 맞추는
  작업은 별도 세션에서 진행한다.
...
- `Theme.uss`의 `--color-bg-scrim-soft`(2.8절) — HUD 5개 컴포넌트가 전부 `--color-bg-panel`을
  쓰면서 소비처 0건(2026-07-17 UI Polish Audit로 확인), 삭제 여부 미결정. **`--color-bg-scrim-
  strong`은 2026-07-22 ResearchPopup의 `modal-scrim`(화면 딤 처리)이 처음으로 실사용 시작 —
  더 이상 미사용 토큰이 아니므로 이 목록에서 제외.**
