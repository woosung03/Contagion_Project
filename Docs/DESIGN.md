...
| `--color-surface-mint` | `rgba(150, 255, 185, 0.04)` | **현재 미사용.** `Theme.uss`에는 정의돼 있지만 소비처(`hero-stat-tile`/`status-row`/`continent-header`)가 이후 라운드에서 배경을 완전히 제거해 실제 참조가 0건이다 — 삭제하지 않고 정의만 남겨둔 상태(부록 B 참고). |
...
| 토큰 | 값 | 사용처 |
|---|---|---|
| `--color-bg-root` | `rgba(8, 8, 16, 1)` | Fullscreen 화면 바닥(MainMenu/CountrySelect) — 0.97이던 시절 뒤에서 계속 렌더링되는 WorldMap이 옅게 비쳐 카드마다 다른 얼룩으로 보이는 문제로 완전 불투명 전환(부록 B 참고) |
...
| `--color-surface` | `rgba(255, 255, 255, 0.08)` | 카드/행 기본 배경(tree-node 등). `pathogen-card`/`country-row`/`status-row`/`detail-panel`은 이후 라운드에서 배경을 제거해 더 이상 이 토큰을 쓰지 않는다(부록 B 참고) |
...
| `pathogen-card` | 없음(투명) | tactical-panel 상속(코너컷 4개) | 0 | Fullscreen 메뉴 카드. 배경은 Direction C 진행 중 제거됨(부록 B) |
| `country-row` | 없음(투명) | 2px `--color-accent-glow-soft` + 좌측 4px | `--radius-md`(6px, 5절 예외) | MainMenu/CountrySelect(7절 Fullscreen). 배경 제거, 외곽 테두리 회색→민트는 Direction C 진행 중 변경(부록 B) |
| `status-row` | 없음(투명) | 좌측 4px severity색(2.2절) + 하단 1px `--color-grid-line` | 0 | CountryStatus 48개국 리스트. 배경 제거·하단 구분선 추가는 Direction C 진행 중 변경(부록 B, 16절) |
...
| `popup-footer-button--confirm` | `background-color: --color-accent-glow-soft; border-color: --color-accent-glow` — 확정 액션 강조(예: "연구 시작", "리더보드 열기") |

**소비처**: `RankingPanel.uss`(확인/닫기), `ResearchPopup.uss`(연구 시작/취소) — 둘 다 로컬
재정의 없이 `Tactical.uss` 정본을 그대로 사용한다.
...
## 14. Data Row System (CountryStatus/ResearchPopup 등 판독 화면 공통)

> 절 제목의 "UpgradeTree"는 2026-07-17 재조사 기준 더 이상 유효하지 않다 — UpgradeTree 캔버스
> 복귀(17절)로 이 화면은 `data-row`를 쓰지 않는다. 아래 노드 상태 모디파이어 항목의 실사용
> 확인 각주 참고.
...
UpgradeTree는 더 이상 `data-row`를 쓰지 않는다(위 실사용 확인 참고).
...
- Hero Stats: 2×2 타일, 타일 폭 48%, 값 폰트 `--font-size-xl`(24px). 배경 없음(투명), 테두리
  `--color-accent-glow`(저반복 4개뿐이라 full 강도). *Direction C 진행 중 변경 — 부록 B 참고.*
- 대륙 아코디언: 코너컷 금지(리스트 반복 요소), 배경 없음(투명), 좌측바 `--color-accent-glow`
  ...
  데이터"가 아니라 "UI 크롬"이라 Severity 4색 사용 금지(2.2절 Don't). *Direction C 진행 중
  변경 — 부록 B 참고.*
- 48개국 리스트 행(`status-row`): 배경 없음(투명), 좌측 4px severity accent-bar(2.2절 4색),
  ...
  `badge-tag--success`/`badge-tag--warning`을 배타 적용한다. *Direction C 진행 중 변경 —
  부록 B 참고.*
...
> **2026-07-17 재조사 — 화면 구조 전면 교체(uncommitted, 작업 중).** Research Database v2의
> 리스트형 UI(`branch-board`/`research-row`/`detail-panel`)는 **전부 제거되었고**, `node-scroll`
> 안에는 다시 Painter2D 절대좌표 캔버스(`tree-node` + 연결선)만 존재한다. 아래 서술은 v1(과거
> "3단계 스케일" 같은 폐기 서술)이 아니라 **현재 working tree의 실제 코드**(`UpgradeTree.uss`/
> `UpgradeTree.uxml`/`UpgradeTreeView.cs`/`TreePathElement.cs`) 기준이다 — 아직 커밋되지 않은
> 진행 중 상태이므로 재조사 시점 이후 다시 바뀔 수 있다. 이전 리스트형 서술은 21.1절에 "제거됨"
> 이력으로만 남긴다.
...
- **연구 상세 팝업**: 노드 클릭 시 `ResearchPopupController.Show()`가 열리고(Overlay Panel,
  11절 Modal System 계약 그대로), 확인 버튼이 `UpgradeManager.TryUnlock()`을 직접 호출한다 —
  성공 시에만 `Hide()`, 실패 시 팝업 유지(실패 사유 텍스트는 CLAUDE.md TODO 커밋 8 범위).
  UpgradeTree 화면 자체는 더 이상 `data-row`/`detail-rows`를 쓰지 않는다(14절 참고) — 수치
  판독은 전부 ResearchPopup 쪽으로 이동했다.
...
| `CountryStatusPanel.uss` | Bottom Sheet(Extended/Dense) | 12.2/16절 소비처. `--color-surface-mint`(2.5절, 현재 미사용) 잔재 및 `.status-row .data-row`(더 이상 어떤 자식도 매치하지 않는 죽은 규칙 — 감염률/사망률이 `status-row__stats-summary` 단일 Label로 교체됨) 정리 대상, 코드 변경 필요(부록 B 참고) |
...
| `UpgradeTree.uss` | Fullscreen, `tree-node` 캔버스(2026-07-17 재조사 기준, uncommitted) | 17절 소비처. `research-row`/`branch-board`/`detail-panel`은 제거됨(21.1절 이력 참고). `tactical-panel`/`corner-cut`/`data-row`/`data-label` 참조는 UXML에서 이미 제거되어 현재 미사용, `data-value--locked/available/active/maxed`(14절)는 정의만 남고 코드가 더 이상 적용하지 않는 죽은 규칙 — 정리 대상 |
| `MainMenu.uss` | Fullscreen, pathogen-card/country-row/detail-panel | 10절 소비처, `--radius-md` 유일 사용처(5절), 21.7절(Pathogen Stat Row) 구현체 |
| `CountrySelect.uss` | MainMenu.uss 확장(개발수준 accent bar 색만 추가) | 10절 소비처 |
| `ResearchPopup.uss` | Overlay Panel(중앙 모달) | 11/11.1절 Modal 계약 소비처. 로컬 중복 정의 없음(modal-footer/popup-footer-button은 Tactical.uss 참조로 정리 완료, UI Audit 1차) |
...
### 21.1 Branch Board — **제거됨(2026-07-17 재조사, uncommitted)**

이 컴포넌트는 더 이상 존재하지 않는다. `branch-board`/`branch-board__rows`/`branch-row`가
UpgradeTree 캔버스 복귀(17절)로 `UpgradeTree.uss`/`UpgradeTree.uxml`에서 완전히 삭제됐다 —
아래는 삭제 전 마지막 스펙을 이력으로만 남긴 것이며, 신규 작업에서 참고/복제하지 않는다.

| 속성 | 값(삭제 전 스펙, 참고용) |
|---|---|
| 목적 | 카테고리당 브랜치 4개(계열 3 + 통합 1)의 해금 진행률(n/m)을 카드 형태로 보여주는 현황판 |
| 사용 화면 | UpgradeTree(Research Database v2, 제거됨) |
| 구성 요소 | `branch-board`(컨테이너, `tactical-panel` 상속) > `branch-board__rows` > `branch-row`(`__label` + `__progress`) |
| 사용 규칙 | `branch-row__progress` 트랙은 `--radius-sm`. `branch-row__progress-fill`은 15절 population-bar와 동일한 `flex-basis:0` + `flex-grow` 비율 기법. 선택 시 `--stroke-selected`(3px) 테두리, 잠김 시 `opacity:0.6` |

같은 이유로 `research-row`(21절 서문이 말하는 "이전 리스트형 UI")와 `detail-panel`
(UpgradeTree 전용 인스턴스 — MainMenu의 동명 컴포넌트와는 별개)도 함께 제거되었다. 현재
스펙은 17절을 본다.
...
| 사용 규칙 | 값 색상은 14절 `data-value--*` severity 모디파이어를 그대로 얹어 재사용 가능(신규 색 도입 없음). 배경 없음(투명), 테두리 `--color-accent-glow`(full) — 10절 참고, Direction C 진행 중 변경(부록 B) |

### 21.3 Quick Stat Grid
...
| 사용 규칙 | 코너컷 부착 금지(8절 "수십 개 반복 리스트" 규칙), 좌측 accent-bar는 `--color-accent-glow`(상시 full — 과거 soft 기본값+hover 시 full 강조 방식이었으나 터치 기기에서 hover가 거의 안 걸려 폐기, Direction C 진행 중 변경) — 대륙 헤더는 "국가 데이터"가 아니라 "UI 크롬"이라 2.2절 severity 4색 사용 금지 |

### 21.7 Pathogen Stat Row (Direction C 진행 중 — MainMenu)
...
> 이탈로 되돌리지도, 2.2절의 공식 예외로 승격하지도 않은 **잠정 상태**다. 상세 경위와 결정
> 대기 항목은 부록 B 참고.
...
## 부록 B. Direction C(Data Wall Terminal) 진행 현황 — 잠정 변경사항, 영구 규칙 아님

**이 부록의 지위**: Direction C는 현재 진행 중인 리디자인이다. 아래 표의 항목은 실제 코드에
반영돼 본문 각 절(10/13/15/16/18/21절)의 값 표에도 동기화했지만, **1절 Design Principles나
2절 Color System의 Do/Don't을 개정하는 새로운 전역 원칙으로는 아직 승격하지 않았다.**
"Approved Exception"이나 공식 예외 정책으로 명문화된 항목은 하나도 없다 — 전부 이 표에서만
추적한다. 본문과 이 부록이 충돌하면(예: 축 분리 원칙 대 21.7절의 severity 재사용) **본문의
핵심 원칙(1~9절)이 우선**하며, 해당 충돌은 "미확정"으로 남겨둔다.

| 라운드 | 화면 | 변경 내용 | 상태 |
|---|---|---|---|
| Color Pass 1 | CountryStatusPanel | hero-stat-tile/status-row/continent-header 배경에 `--color-surface-mint`(신규 토큰) 적용 | **폐기됨** — Color Pass 2가 배경 자체를 없애며 대체, 토큰은 미사용 상태로 잔존(2.5절) |
| Color Pass 2 | CountryStatusPanel/MainMenu/CountrySelect | hero-stat-tile/status-row/continent-header/pathogen-card/detail-panel/country-row 배경 완전 제거, `--color-bg-root` 0.97→1(지도 유출 방지), country-row 테두리 회색→`--color-accent-glow-soft` | 본문 반영 완료(10절/2.7절/16절) |
| 밀도 개선(Tier A) | CountryStatusPanel | status-row padding/margin 축소(토큰화), badge-tag에 `--font-size-caption-2xs`+bold 최초 적용 | 본문 반영 완료(3절) |
| 정보 밀도 | MainMenu | `pathogen-card__stat-value`(막대 옆 % 표시) 신규 | 본문 반영 완료(21.7절) |
| Color Pass 4 | MainMenu | `pathogen-card__stat-fill--severity/lethality/resistance`가 severity 4색(2.2절 "국가 데이터 전용") 재사용 | **미확정** — 21.7절 참고, 정책 결정 대기 |
| Color Pass 5A | CountryStatusPanel | status-row 하단 1px hairline(`--color-grid-line`) 추가, 대륙 아코디언 좌측바 soft→full 승격, 배지에 `ApplySeverityClass()` 적용(공항/항구/국경 색상 신호) | 본문 반영 완료(10절/16절/13절/21.6절) |
| Phase 5B | CountryStatusPanel | 감염률/사망률 `data-row` 2줄 → `status-row__stats-summary` 1줄 요약("감염 N% · 사망 N%", `population-bar__summary` 문법 재사용) | 본문 반영 완료(16절) |

**미확정 항목(정책 결정 필요)**:
- **severity 축의 병원체 스탯 재사용**(위 표 Color Pass 4) — (a) 2.2절에 예외 각주를 추가해
  공식화하거나, (b) 다른 축(예: 브랜드색 계열)으로 되돌리는 두 갈래 중 아직 결정되지 않았다.
  결정 전까지 코드는 유지하되 본문 원칙(2.2절)은 개정하지 않는다.

**코드 정리 대상(문서 범위 밖, 별도 승인 필요)**:
- `CountryStatusPanel.uss`의 `.status-row .data-row` 규칙 — Phase 5B로 더 이상 어떤 자식
  요소도 매치하지 않는 죽은 코드(18절 참고).
