...
## Step 85 — SafeArea Minimum Margin Fix

- **문제**: `Theme.uss`에 Spacing Scale(`--space-xs`~`--space-xxl`, 4~24px)이 있지만 실기 플레이 시
  버튼/패널/HUD/UpgradeTree/MainMenu가 화면 끝에 붙어 보임. 원인은 Spacing System이 아니라
  `SafeAreaApplier`가 `ApplyDevicePx()`에서 safe-area inset을 그대로 `target.style.padding*`에
  덮어써서, 노치 없는 기기(inset=0)에서는 좌우/상하 padding이 0으로 붕괴하는 것 — `UpgradeTree.uss`의
  `.upgrade-root { padding: var(--space-xl); }` 같은 USS 기본값조차 런타임에 무시됐다.
- **수정 범위**: `Assets/Scripts/UI/SafeAreaApplier.cs` 단일 파일. `Theme.uss`/`RankingPanel`/
  `CountryPopup`/`CountryStatusPanel`/HUD 등은 손대지 않음(이번 작업은 SafeArea 버그 수정으로 한정,
  Spacing System 재설계 아님).
- **구현**: `minMargin`(기본 20f) 필드 신규 추가. `ApplyDevicePx()`의 좌/우/상/하 각 padding 계산을
  `Mathf.Max(minMargin, inset)`으로 변경 — safe-area inset이 minMargin보다 크면(노치 기기) inset이
  그대로 쓰이고, 작으면(대부분의 안드로이드·구형 기기) minMargin이 최소 여백을 보장한다. 상단은
  nav바 회피 여백(`navBarReserveTop`, 기본 48)을 `Mathf.Max(minMargin, topInset)` 이후 별도로 더해
  기존 동작을 그대로 보존. 에디터 `ApplyPreview()` 경로도 동일 공식 적용해 Preview/실기기 결과가
  갈리지 않게 함.
- **minMargin 값 근거**: 12/16/20px 후보 중 20px(`--space-xl`) 선택 — `UpgradeTree.uss`의
  `.upgrade-root`가 이미 이 값을 화면 루트 padding 기본값으로 쓰고 있었고(런타임에 덮어써져 무시된
  바로 그 값), `RankingPanel.uss` 루트는 24px(`--space-xxl`)를 쓰지만 그건 패널 내부 padding이지
  화면 최외곽 루트가 아님. 화면 최외곽 루트 기준 기존 컨벤션과 일치하는 20px을 채택.
- **디버그 오버레이**: `BuildOverlayText()`에 `minMargin` 값 표시 추가(실기기 검증용, 기능 변경 없음).
- **검증**: Unity 에디터 미접속이라 실기 확인은 못 함. 코드 리뷰로 로직 확인 — `QA_Checklist.md`에
  검증 항목 추가 필요.
