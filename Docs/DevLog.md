...
## Step 82 — Research Database UI Shell (Commit 1, 데이터 미연결)

- 근거 문서: `Docs/ResearchDatabase_MVP_ImplementationPlan.md`, `Docs/UpgradeTree_ResearchDatabase_ScreenDesign.md`.
  사용자가 이번 커밋 범위를 명시적으로 좁힘 — "화면 전환 + UI 구조 변경"만, 실제 `UpgradeNode`
  연결은 Commit 2로 이연. `environmentResistance`/`medicalBurdenModifier`/`unlockedFlags`/신규
  시뮬레이션 시스템/신규 데이터 모델은 이번 커밋 범위에 전혀 포함되지 않음(계획서 §0 제외 목록과
  동일 원칙).
- **UI 구조 변경**: `UpgradeTreeView.cs`의 절대좌표 캔버스 렌더링(`RebuildTree()`의 좌표 계산,
  `DrawConnections()`의 Painter2D 꺾은선/포트 마커)을 전부 제거하고 `BuildResearchList()`(세로
  스크롤 리스트 — 갈래 섹션 헤더 + 연구 항목 행)로 교체. `UpgradeTree.uxml`의 좌우 화살표
  (◀▶ `OnPrevRequested`/`OnNextRequested`) 내비게이션을 상단 고정 탭 3개(전파/증상/적응)로
  교체 — 탭이 상시 노출되어 화살표가 갖던 "지금 몇 번째 카테고리인지 안 보임" 문제가 구조적으로
  해소됨.
- **더미 데이터**: 이번 커밋은 `UpgradeManager.Instance.Tree`를 전혀 읽지 않는다. 브랜치·연구
  항목명은 `NodeDisplayNames`/`ScreenDesign.md` §3 예시 문구를 그대로 따서(예: 전파 탭 — 공기
  계열/수인성 계열/접촉·동물매개 계열/통합 연구, 각 계열당 2~4개 항목) 각 카테고리(전파/증상/적응)
  별로 하드코딩한 `BuildDummyBranches()`가 제공한다. 4가지 상태(잠김/연구 가능/진행 중/완료)를
  전부 최소 1개씩 포함해 상태별 색상(`research-row--locked/available/active/maxed`)이 전부
  보이도록 구성. 상세 패널의 "연구 시작" 버튼은 실제 노드에 연결돼 있지 않으므로 항상
  비활성화(`SetEnabled(false)`) — Commit 2에서 `UpgradeManager.TryUnlock()` 연결과 함께 활성화.
- **"능력" → "적응" 개명**: `ResearchDatabase_Design.md` 권장안을 반영해 `CategoryLabel()`/
  `CategoryPrefix()`의 `UpgradeCategory.Ability` 표시 문자열을 "능력"에서 "적응"으로 변경(enum
  `Ability` 자체는 무변경, 표시 문자열만 교체 — 계획서 §1 Step 3과 동일 원칙). 상태 라벨도
  `ScreenDesign.md` §5 표에 맞춰 "해금 가능"→"연구 가능"으로 텍스트만 조정.
- **3-GameObject 구조 유지**: `TransmissionTreeUI`/`SymptomTreeUI`/`AbilityTreeUI` 독립
  UIDocument 3개 구조는 그대로 유지(계획서 §6-3 권장안 채택 — 씬 배선 재작업 회피). 대신
  `UpgradeTreeView.OnPrevRequested`/`OnNextRequested`(인자 없는 이벤트 2개)를
  `OnCategoryRequested(UpgradeCategory)`(단일 이벤트) 하나로 교체하고, `UIManager`가
  `HandleCategoryRequested()`에서 요청된 카테고리를 담당하는 `UpgradeTreeView`를 `Category`
  프로퍼티(신규 public getter)로 찾아 그 화면만 보이고 나머지 둘은 닫는다 — 탭 클릭 시 세
  UIDocument의 표시 여부만 토글되는 셈이라 사용자에게는 화면 하나에서 탭이 전환되는 것처럼 보인다.
- **재사용/보존**: `NodeDisplayNames`/`DisplayName()`/`_codeByNodeId`/`DetermineState()`/
  `EffectStatLabel()`은 이번 커밋에서 미사용이지만 삭제하지 않고 그대로 남겨둠 — Commit 2에서
  실제 `UpgradeManager.Tree` 연결 시 재사용 예정(계획서 §5 재사용 표와 동일). `UpgradeNode.cs`/
  `UpgradeManager.cs`/`DefaultUpgradeTreeFactory.cs`/`UpgradeTreeDatabase.cs`는 전혀 손대지 않음.
- **미검증**: Unity 에디터 미접속으로 실제 렌더링/탭 전환/스크롤 동작은 확인 못 함 — 검증 항목은
  `Docs/QA_Checklist.md`에 추가.
- **변경 파일**: `Assets/Scripts/UI/UpgradeTreeView.cs`, `Assets/UI/UpgradeTree.uxml`,
  `Assets/UI/UpgradeTree.uss`, `Assets/Scripts/Managers/UIManager.cs`.
- **다음 단계(Commit 2, 이번 범위 아님)**: 더미 브랜치/항목을 `DefaultUpgradeTreeFactory.cs`의
  실제 45개 노드(`prerequisites`/`position` 기준 그룹핑)로 교체, `_codeByNodeId`/`DetermineState()`
  실제 연결, "연구 시작" 버튼 `UpgradeManager.TryUnlock()` 연결.

## Step 83 — Research Database v2 커밋 5 (실제 UpgradeNode 연결)

- 근거 문서: `Docs/ResearchDatabase_V2_ImplementationPlan.md` §1 Step 8~12, §4 커밋 5. Step 82
  이후 커밋 2~4(branch-board UXML/USS 골격, `ResearchPopup.uxml`/`.uss` 신규, `NodeDisplayNames`/
  `NodeDescriptions`/`NodeBranch` 45개 노드 메타데이터, `ResearchPopupController` 뼈대)는 이번
  세션 이전에 이미 완료돼 있었으나 DevLog에 별도 Step으로 기록되지 않은 채 진행됨 — 이번 Step은
  그 다음 단계(커밋 5)만 다룬다.
- **범위**: `Assets/Scripts/UI/UpgradeTreeView.cs` 한 파일만 수정. 사용자가 명시적으로 범위를
  좁혔다 — Research Popup 이벤트 배선(`OnResearchItemSelected`), `ResearchPopupController` 수정,
  `UIManager` 수정, "연구 시작" 구매 로직(`UpgradeManager.TryUnlock()`) 연결, 비용 공식 변경은
  전부 금지(계획서 커밋 6~7 이후 범위).
- **제거**: `DummyResearchItem`/`DummyBranch` 클래스, `BuildDummyBranches()`, `SelectDummyItem()`,
  `_selectedDummyItem` 필드 전부 삭제.
- **실제 연결**: `UpgradeManager.Instance.Tree`를 `NodeBranch`(커밋 3에서 이미 존재) 기준으로
  그룹핑하는 `BuildBranches()` 신규 — 카테고리별 브랜치 순서는 `BranchOrder`(신규 딕셔너리,
  계열 3 + 통합 연구 1)로 고정하고, 브랜치 내부는 `position.y`(티어) → `position.x`(열) 순 정렬.
  이미 선언만 되고 미사용이던 `_codeByNodeId`/`DetermineState(UpgradeNode)`를 실제로 연결(전자는
  `AssignCodes()`가 브랜치 선택과 무관하게 카테고리 전체를 훑어 코드를 고정 배정).
- **브랜치 보드/리스트**: `BuildBranchBoard()`가 `branch-board__rows`(커밋 2에서 이미 UXML에
  추가된 빈 컨테이너)를 브랜치별 진행률(n/m) 행으로 채움 — 진행률 막대는 population-bar류와
  달리 세그먼트가 2개(해금/잔여)뿐이라 새 USS 클래스를 추가하지 않고 인라인 `style.flexGrow`로만
  비율을 잡았다(이번 커밋이 `.cs` 파일 하나만 건드리는 원칙을 지키기 위함). `BuildResearchList()`는
  전체 브랜치 대신 `SelectBranch()`로 선택된 브랜치 하나만 그리도록 축소.
- **하단 상세 패널**: 개별 항목 상세 대신 "선택된 브랜치" 요약 2행(진행률, 다음 추천 연구)으로
  전환(`BuildBranchSummary()`) — 개별 항목 상세는 Research Popup(커밋 6~7)의 역할로 남겨둠. 연구
  항목 행은 이번 커밋에서 클릭 핸들러를 붙이지 않았다(탭해도 동작 없음 — 팝업 배선 전까지의 안전한
  중간 상태, 기존 커밋들의 "빈 컨테이너만 두는" 전략과 동일).
- **비용 텍스트**: `CostText()` 신규 — locked는 "—", available은 `UpgradeManager.GetEffectiveCost()`
  (읽기 전용 조회, 비용 로직 자체는 무변경), active/maxed(둘 다 이미 해금됨)는 "완료". 구 더미
  데이터의 "active" 상태가 실제 비용을 보여줬던 것은 `DetermineState()`의 실제 의미(이미 해금된
  중간 티어 노드)와 맞지 않는 표기였음을 이번에 바로잡았다.
- **검증**: Unity 에디터 미접속이라 실제 렌더링은 확인 못 함. 대신 파이썬으로 (1) 중괄호 balance,
  (2) 삭제된 Dummy 타입/메서드 참조 잔존 여부, (3) `NodeDisplayNames`/`NodeDescriptions`/`NodeBranch`
  3개 딕셔너리가 `DefaultUpgradeTreeFactory.cs`의 45개 노드를 전부 커버하는지, (4) `NodeBranch`와
  `BranchOrder`의 라벨 문자열이 완전히 일치하는지, (5) 카테고리별 브랜치 분포가 정확히 4+4+4+3으로
  떨어지는지를 정적으로 검증 — 전부 통과. 남은 렌더링/클릭 검증은 `Docs/QA_Checklist.md` 참고.
- **변경 파일**: `Assets/Scripts/UI/UpgradeTreeView.cs` (단일 파일).
- **다음 단계(커밋 6~7, 이번 범위 아님)**: `OnResearchItemSelected(UpgradeNode)` 이벤트 신규 →
  연구 항목 행 클릭 시 발행(커밋 6), `UIManager`가 3개 `UpgradeTreeView`를 구독해
  `ResearchPopupController.Show()` 호출 + `UpgradeManager.OnNodeUnlocked` 구독으로 활성 뷰 갱신
  (커밋 7). 계획서 §4 참고.

## Step 84 — Research Database v2 커밋 6 (연구 항목 행 클릭 이벤트 배선)

- 근거 문서: `Docs/ResearchDatabase_V2_ImplementationPlan.md` §1 Step 14, §4 커밋 6. Step 83의
  "다음 단계" 그대로 이어감.
- **범위**: `Assets/Scripts/UI/UpgradeTreeView.cs` 한 파일만 수정. `ResearchPopupController.cs`/
  `UIManager.cs`는 손대지 않음(구독 배선은 커밋 7).
- **이벤트 추가**: `public event System.Action<UpgradeNode> OnResearchItemSelected` 신규 —
  기존 `OnCategoryRequested` 바로 아래 선언.
- **배선**: `CreateResearchRow(UpgradeNode node)`가 만드는 `research-row`에
  `RegisterCallback<ClickEvent>(_ => OnResearchItemSelected?.Invoke(node))` 추가. LOCKED 상태를
  포함한 모든 상태에서 발행하도록 결정 — 잠긴 항목도 상세 팝업에서 선행조건을 확인할 수 있어야
  하고, "연구 시작" 버튼 활성화 여부는 팝업(커밋 7 이후) 쪽 책임으로 분리했기 때문에 행 클릭
  자체를 상태로 막을 이유가 없다.
- **구독자 없음 확인**: `UIManager`가 아직 이 이벤트를 구독하지 않으므로 지금 연구 항목을 탭해도
  화면상 변화는 없다(컴파일만 통과하는 안전한 중간 상태 — 계획서가 의도한 커밋 6/7 분리와 동일).
- **검증**: Unity 에디터 미접속이라 실제 클릭 동작은 확인 못 함. 파이썬으로 중괄호/괄호 balance만
  정적 검증 — 통과. 실제 클릭 시 이벤트 발행 여부는 `Docs/QA_Checklist.md`에 커밋 7(팝업 연결)
  완료 후 함께 검증 항목으로 추가 예정.
- **변경 파일**: `Assets/Scripts/UI/UpgradeTreeView.cs` (단일 파일).
- **다음 단계(커밋 7, 이번 범위 아님)**: `UIManager.cs`에 `researchPopupController` 필드 추가,
  3개 `UpgradeTreeView.OnResearchItemSelected` 전부 구독해 `ResearchPopupController.Show()` 호출,
  `UpgradeManager.OnNodeUnlocked` 구독으로 활성 뷰의 브랜치 보드/요약 갱신 — 이 커밋부터 Research
  Popup이 실제로 동작. 계획서 §4 커밋 7 참고.

## Step 86 — CountryStatusPanel 48개국 목록 → 대륙별 접기/펼치기 아코디언

- 근거: 사용자 요청(2026-07-14, Research Database v2와 무관한 별도 UX 개선). 48개국 단일 리스트가
  스크롤이 너무 길다는 문제를 대륙별 그룹핑 + 접기/펼치기로 해소.
- **대륙 데이터 소스**: `Country`(`Assets/Scripts/Data/Country.cs`)에는 대륙/지역 필드가 원래
  없다(조사 완료, lat/lon도 없음 — 지리 좌표는 `CountryView`/`WorldMap` 등 뷰 레이어에만 존재).
  데이터 모델(`Country.cs`, `CountryDatabase.asset`)은 건드리지 않고
  `CountryStatusPanelController` 전용 `Dictionary<string,string> ContinentByCountryId`(id→대륙
  라벨, 48개 전부 static 매핑)로 처리 — `CaseFatalityRate()`/`MedicalLoad()`를
  `CountryPopupController`에서 독립적으로 복제해온 이 파일의 기존 관례와 동일한 방식(두 화면이
  서로 의존하지 않게).
- **대륙 분류**: 6대륙 단순화(ASIA 19/EUROPE 7/NORTH AMERICA 3/SOUTH AMERICA 4/AFRICA 14/
  OCEANIA 1 = 48개국 전부 매핑 확인). 러시아→EUROPE, 튀르키예/사우디/이란/이라크/예멘 등 중동
  국가→ASIA로 편의상 분류(대륙에 걸친 나라는 통상적 6대륙 단순화 관례를 따른 판단값 — 실제
  플레이 확인 후 조정 가능, 이견 있으면 `ContinentByCountryId` 딕셔너리 값만 바꾸면 됨).
- **구현**: `EnsureRowsBuilt()`가 기존처럼 최초 1회만 실행되는 가드를 그대로 유지한 채, 국가 행을
  `_statusList`에 바로 붙이는 대신 `BuildContinentSection()`이 만든 대륙 섹션(헤더+바디)에 담아
  붙이도록 변경. 헤더 클릭 → `ToggleContinent()` → `_continentExpanded` 갱신 →
  `ApplyContinentExpandedState()`가 바디 `display`(Flex/None)와 화살표(▼/▶) 텍스트를 갱신. 펼침
  상태는 컨트롤러 인스턴스 필드에 남아있고 섹션이 최초 1회만 생성되므로, `CountryPopup`을 열고
  닫아도(=이 컨트롤러가 파괴되지 않는 한) 별도 저장 로직 없이 유지된다.
- **유지된 부분**: `OnCountryRowSelected` 이벤트/발행 위치(`BuildRow()`)·`_rowsByCountryId` 캐싱·
  `RefreshRow()` 틱 갱신 로직 전부 무변경 — 국가 행 자체의 클릭 동작과 데이터 갱신 흐름은 건드리지
  않고 "어느 컨테이너에 담기는가"만 바뀌었다.
- **스타일**: `CountryStatusPanel.uss`에 `.continent-section`/`.continent-header`/
  `.continent-header__arrow`/`.continent-header__label`/`.continent-body` 신규 추가. 코너컷은
  쓰지 않고(DESIGN.md Corner Cut Don't — 반복 리스트 행) `accent-bar-row`(Tactical.uss)만
  재사용, 색상은 severity 4색이 아니라 구조용 발광색(`--color-accent-glow(-soft)`)을 사용(대륙
  헤더는 "국가 데이터"가 아니라 "UI 크롬"이므로 DESIGN.md Severity Colors Don't 규칙 적용).
  `Theme.uss`의 카드류 공통 hover 트랜지션 선택자 목록(`.pathogen-card, .country-row, .tree-node,
  .status-row`)에 `.continent-header`를 추가해 기존 DRY 패턴을 그대로 따름.
- **검증**: Unity 에디터 미접속이라 실제 렌더링/클릭은 확인 못 함. 대신 (1) `ContinentByCountryId`
  48개 항목이 `CountryDatabase.asset`의 48개 id와 정확히 1:1 일치(누락/초과 없음, 중복 없음)하는지,
  (2) 대륙별 개수 합이 48인지, (3) 컨트롤러 코드의 중괄호/괄호 balance를 파이썬으로 정적 검증 —
  전부 통과. 실제 펼침/접힘 토글, 국가 행 클릭 → 팝업 연결, ScrollView 동작은
  `Docs/QA_Checklist.md`에 검증 항목으로 추가 필요.
- **변경 파일**: `Assets/Scripts/UI/CountryStatusPanelController.cs`, `Assets/UI/CountryStatusPanel.uss`,
  `Assets/UI/Theme.uss`(공유 hover 선택자 1줄 추가). `CountryStatusPanel.uxml`은 무변경(`status-list`
  컨테이너를 그대로 재사용, 대륙 섹션은 `AddDataRow()`와 같은 방식으로 런타임에 조립).

## Step 85 — CountryStatusPanel을 Research Database와 같은 리스트→상세 팝업 구조로 확장

- **배경**: Step 67에서 "국가 탭 시 Country Dock과 CountryPopup이 동시에 뜬다"를 미결정 버그로
  남겨뒀었다(자동 팝업 유지 vs Country Dock으로 대체). 이번 세션에서 Research Database 구조
  조사(`UIManager`/`UpgradeTreeView`/`ResearchPopupController` 흐름 분석) 후 사용자가 다음을
  확정 결정했다: 지도 클릭 CountryPopup 유지, `CountryDockController` 유지, `AppScreen.GlobalStatus`
  유지, **CountryPopup을 지도 클릭과 CountryStatusPanel 양쪽에서 재사용**, 새 Country Database
  화면은 만들지 않고 기존 `CountryStatusPanel`을 리스트→상세 팝업 구조로 확장. 즉 Step 67의
  "동시에 뜨는" 동작은 버그가 아니라 유지하기로 결정됐고, 대신 CountryStatusPanel의 48개국 목록도
  같은 `CountryPopupController`를 여는 두 번째 진입점으로 삼는 방향으로 풀었다.
- **구조 매핑**: Research(`UpgradeTreeView` 리스트 → `OnResearchItemSelected` → `UIManager` →
  `ResearchPopupController.Show()`)와 동일한 패턴을 그대로 복제 — `CountryStatusPanelController`
  (리스트) → `OnCountryRowSelected` → `UIManager` → `CountryPopupController.ShowCountry()`.
- **변경 파일 4개**:
  - `Assets/Scripts/UI/CountryStatusPanelController.cs` — `public event Action<Country>
    OnCountryRowSelected` 추가, `BuildRow(Country)`가 만드는 `status-row`에
    `RegisterCallback<ClickEvent>(_ => OnCountryRowSelected?.Invoke(country))` 배선(research-row와
    동일하게 상태와 무관하게 항상 클릭 가능).
  - `Assets/Scripts/UI/CountryPopupController.cs` — 기존 `HandleCountryClicked(Country)`의 본문을
    공개 메서드 `ShowCountry(Country)`로 추출하고 `HandleCountryClicked`는 이를 호출하도록 변경.
    지도 클릭 경로와 리스트 클릭 경로가 완전히 같은 코드를 탄다.
  - `Assets/Scripts/Managers/UIManager.cs` — `countryStatusPanelController.OnCountryRowSelected`
    구독 추가(OnEnable/OnDisable), `HandleCountryRowSelected(Country)` 핸들러 신규 —
    `countryPopupController?.ShowCountry(country)` 호출만 함. `WorldMapInputLock` 처리는 추가하지
    않았다 — `CountryPopupController`는 원래부터 이 락 시스템 밖에 있고(설계 문서 참고), 이번
    변경으로도 그 방침을 바꾸지 않았다.
  - `Assets/Scenes/GamePlay.unity` — **`CountryPopupUI` UIDocument의 `m_SortingOrder`를 1 → 2로
    변경**. 씬을 직접 조사한 결과 `CountryPopupUI`와 `CountryStatusPanelUI`가 동일
    `PanelSettings`(guid `da1e200d22ea5964385c0686b12834de`) + 동일 `m_SortingOrder: 1`을 쓰고
    있었다 — `CountryPopupController.cs` 기존 주석에 이미 "두 화면이 동시에 열리면 어느 쪽이
    위에 오는지 불명확하고, CountryStatusPanel의 전체화면 루트가 CountryPopup의 닫기 버튼 클릭을
    가로챈다"고 기록돼 있던 바로 그 충돌이다. 지금까지는 `CountryStatusPanelController`가 지도
    클릭 시 스스로 닫혀버려(`HandleWorldMapCountryClicked`) 두 화면이 실제로 동시에 열릴 일이
    없어서 드러나지 않았는데, 이번 결정으로 "GlobalStatus 화면이 열린 채로 그 안의 리스트를 눌러
    팝업을 여는" 시나리오가 의도적으로 생기므로 반드시 고쳐야 했다. `EndingScreenUI`만
    `m_SortingOrder: 2`를 쓰고 있어(항상 최상단이어야 하는 게임오버 화면) 그 값을 그대로
    재사용했다 — EndingScreen과 CountryPopup은 동시에 열릴 일이 없어(EndingScreen 진입 시
    `GameManager.SetPaused`로 게임이 멈추고 다른 화면 트리거가 발생하지 않음) 충돌 없음.
- **의도적으로 손대지 않은 것**: `HandleWorldMapCountryClicked`(지도 클릭 시 GlobalStatus 자기
  자신을 닫는 로직)는 그대로 유지했다 — 이건 "지도를 직접 클릭했을 때"의 동작이고, 이번에 추가한
  건 "리스트 행을 클릭했을 때"의 별도 트리거라 서로 간섭하지 않는다. `status-row`에 hover 등
  신규 시각 피드백 CSS는 추가하지 않았다 — `research-row`도 hover 없이 상태 색상(border-left)만으로
  피드백을 주는 동일 규약이라 통일성을 위해 그대로 따름.
- **미해결로 남긴 것**: 설계 보고서(이전 세션 결과)에서 지적한 "감염자 상위/사망자 상위 랭킹"과
  현재 "종합 위협도 TOP10 + 감염자 TOP10" 통합 사이의 불일치, 화면 타이틀("GLOBAL STATUS CENTER")
  명칭 변경 여부는 이번 범위에서 다루지 않았다 — 사용자가 확정한 결정 목록에 포함되지 않음.
- **검증**: Unity 에디터 미접속 — 실제 클릭/렌더 순서 확인은 QA_Checklist.md에 항목 추가 후
  플레이테스트로 확인 필요.

## Step 88 — CountryStatusPanel v4 Performance Dashboard 재설계 (정체성 최종 확정)

- **배경**: Step 87의 "플레이어 의사결정 콘솔" 가정을 사용자가 재검토 후 뒤집었다(2026-07-15,
  같은 날 두 번째 재설계). 분석 결과 플레이어는 이 화면에서 "다음에 무엇을 해야 하는가"가
  아니라 "내가 얼마나 잘하고 있는가"를 확인하고 싶어한다는 결론 — CountryStatusPanel의 정체성을
  **"전파 전략 콘솔" → "성과 대시보드"**로 최종 확정했다. 역할 분리: WorldMap=전략 판단,
  CountryPopup=국가 상세 브리핑, UpgradeTree=업그레이드 의사결정, CountryStatusPanel=성과/진행
  상황 확인. 전략 정보(공항/항구/국경/봉쇄/전략 오버레이)는 이 화면의 정체성에서 제외하고 향후
  WorldMap 개선 작업으로 이관하기로 했다 — 단 이번 작업 범위는 아니다.
- **확정 순서(위→아래)**: GLOBAL STATUS(유지) → **WORLD OVERVIEW**(세계 요약+감염 국가 현황+
  국가 상태 분포+의료 시스템 현황을 캡션 하나로 통합) → 감염자 TOP10(유지) → **사망자 TOP10
  (신규)** → 48개국 목록(최상단→최하단, "핵심 콘텐츠"가 아니라 "상세 조회 도구"로 재정의).
  Step 87에서 최상단으로 옮겼던 48개국 목록과 종합 위협도 TOP10을 다시 뒤집는 결정 — 종합
  위협도 TOP10은 "전략 콘솔" 시절 설계라 성과 대시보드 관점에서 부적절해 완전히 제거했다.
  Step 85 "미해결로 남긴 것"에서 지적했던 "감염자 상위/사망자 상위 랭킹 vs 종합 위협도 통합"
  불일치가 이번에 사망자 TOP10 추가로 해소됐다.
- **UXML 변경**: 캡션 4개(세계 요약/감염 국가 현황/국가 상태 분포/의료 시스템 현황) → 캡션
  1개(WORLD OVERVIEW)로 통합(사용자 확인: 각 블록이 이미 자체 라벨을 가지고 있어 중복 캡션
  불필요, "통합" 의도에 부합). 감염 국가 현황 3행(`infection-summary-rows`)이 기존 최하단에서
  population-bar 세계 요약 바로 다음으로 이동한 것이 유일한 내부 순서 변경. "종합 위협도 TOP 10"
  캡션+`ranking-threat` 컨테이너 삭제. "사망자 TOP 10" 캡션+`ranking-dead`(`detail-rows` 클래스
  재사용) 신규 추가. "48개국 목록" 섹션 전체를 스크롤 최하단으로 이동. 신규 CSS 없음 — 기존
  `modal-section-caption`/`detail-rows`/`distribution-row`/`population-bar` 재사용.
- **Controller 변경**: `_rankingThreat` 필드/쿼리/`PopulateRankings()` 내 위협도 랭킹 호출
  제거. `_rankingDead` 필드/쿼리 추가, `PopulateRankings()`에 `RebuildTopRows(_rankingDead,
  countries, c => (double)c.deadCount, c => $"{c.deadCount:N0}", _ => "data-value--dead")`
  추가(감염자 TOP10과 동일 구조, 신규 계산식 없음, 기존 `Country.deadCount` 재사용). `ThreatIndex()`
  가 제거되면서 그 함수가 유일하게 참조하던 `CaseFatalityRate(Country)`도 다른 곳에서 미사용이
  되어 함께 삭제했다(사용자 확인) — 세계 요약의 "치사율(추정)" 행은 `PopulateWorldSummary()`
  안의 별도 인라인 계산(`worldCfr`)이라 영향 없음. `InfectionRatio()`/`MedicalLoad()`/
  `MedicalLoadStatus()`/`RefreshRow()`/`PopulateWorldSummary()`/`PopulateDistribution()`/
  `PopulateMedicalDistribution()`/`PopulateInfectionSummary()`/`EnsureRowsBuilt()` 등 나머지
  로직·바인딩은 전혀 변경 없음 — `OnEnable()`의 `root.Q<T>("name")` 조회가 이름 기반이라 UXML
  순서 변경과 무관.
- **의도적으로 유지한 것(사용자 확인)**: 48개국 목록 각 행의 공항/항구/국경 표시(`FlagsLabel`,
  `status-row__detail`)는 "전략 정보 제거" 조항이 신규 전략 오버레이 추가 금지를 의미하는
  것으로 판단해 그대로 유지했다 — 목표 4("국가 목록 재배치... 기존 기능 유지")와 일치.
- **이번 범위에서 다루지 않은 것**: GLOBAL STATUS 배너 문구("CONTAINMENT SUCCESS" 등)가 인류
  관점(봉쇄 성공=긍정)으로 쓰여 있어 "플레이어=전염병" 정체성과 반대 방향이라는 점을 확인했으나,
  사용자 지시에 따라 문구 변경 여부 보고만 하고 실제 수정은 하지 않았다 — 필요 시 별도 작업으로
  제안.
- **검증**: Unity 에디터 미접속 — WORLD OVERVIEW 통합 레이아웃/사망자 TOP10 정렬/48개국 목록
  최하단 스크롤 위치는 QA_Checklist.md 항목 추가 후 플레이테스트로 확인 필요.

## Step 87 — CountryStatusPanel UX Reorder ("세계 통계 리포트" → "플레이어 의사결정 콘솔")

- **배경**: 사용자 UX Audit 결과(2026-07-15) 확정. 실제 플레이어는 이 화면을 "세계 통계를 읽는
  리포트"가 아니라 "다음 행동을 정하는 의사결정 콘솔"로 쓰는데, 가장 자주 쓰는 정보(48개국 목록·
  종합 위협도 TOP10)가 최하단에, 실행 가능성이 낮은 집계 정보(세계 요약·분포·의료 현황·감염
  국가 현황)가 상단 대부분을 차지하고 있었다. 새 기능/UI/색상 추가 없이 **섹션 순서만** 재배치.
- **확정 순서(위→아래)**: GLOBAL STATUS(유지) → 48개국 목록(최하단→최상단 그룹) → 종합 위협도
  TOP10 → 감염자 TOP10(유지, 삭제/대체 없음) → 세계 요약 → 국가 상태 분포 → 의료 시스템 현황 →
  감염 국가 현황(최하단).
- **분석 결과 — UXML 순서만 변경으로 충분, Controller 로직 변경 불필요**:
  - `CountryStatusPanelController.OnEnable()`의 모든 필드 바인딩이 `root.Q<T>("name")` 이름 기반
    조회라 UXML 자식 순서와 무관 — 재배치해도 바인딩 깨지지 않음.
  - `HandleWorldStateChanged`/`PopulateWorldSummary`/`PopulateDistribution`/`PopulateMedicalDistribution`/
    `PopulateRankings`/`PopulateInfectionSummary`/`PopulateList`/`EnsureRowsBuilt` 전부 캐시된
    필드 참조에 직접 쓰기만 하고, 서로의 실행 순서나 DOM 위치에 의존하지 않음(각 메서드가 자기
    컨테이너만 `Clear()`+재생성).
  - `CountryStatusPanel.uss`에 순서 의존 셀렉터(`:last-child`, `flex-grow`를 특정 위치에 기대는
    값, `margin-top: auto` 등) 없음 — `status-scroll`은 단순 `flex-direction: column` 세로 스택.
  - 결론: **`CountryStatusPanel.uxml`의 `ui:ScrollView` 자식 블록 8개 순서만 재배치**, Controller/
    USS는 무변경으로 충분하다고 판단(실제로 무변경).
- **구현**: `CountryStatusPanel.uxml`의 `status-scroll` 내부 8개 섹션 블록(캡션+콘텐츠 쌍)을
  확정 순서대로 잘라 붙였다. 각 섹션 내부 마크업(요소 `name`/`class`)은 전혀 건드리지 않음 —
  이동한 블록만 통째로 옮기고 순서 변경 이유를 인라인 주석(`[UX Reorder, 2026-07-15]`)으로
  남김. `CountryStatusPanelController.cs`의 클래스 상단 XML 문서 주석("화면 구성(위→아래)" 설명)도
  실제 순서와 어긋나게 되므로 새 순서로 갱신(로직 변경 없음, 문서 동기화만).
- **의도적으로 손대지 않은 것**: 색상/버튼/Stroke/Density Mode/신규 컴포넌트 — 전부 무변경.
  감염자 TOP10 삭제 안 함, 청정 국가·최후 저항 국가·새 랭킹 추가 안 함(사용자 명시 금지 항목).
- **잠재적 부작용**: (1) 대륙 아코디언(`EnsureRowsBuilt()`)이 패널 최초 `Show()` 시 1회만 실행되는
  가드는 그대로라 순서 변경과 무관하게 기존 펼침 상태 유지 동작 그대로. (2) `status-scroll`의
  `vertical-scroller-visibility="Hidden"`이라 스크롤바가 안 보이는 채로 세로 스크롤만 되는 기존
  동작도 콘텐츠 총량이 그대로라(섹션 8개 순서만 바뀜, 개수/크기 불변) 변화 없음. (3) 48개국
  목록이 최상단으로 오면서 패널을 열자마자 국가 리스트가 먼저 보이게 되는데, `EnsureRowsBuilt()`가
  `Show()`에서 항상 호출되므로 최초 스크롤 위치에서 아코디언 헤더(대륙 6개)가 바로 보이는 것은
  의도된 동작(요구사항의 목적 자체가 "가장 자주 쓰는 것을 먼저 보이게" 이므로 side effect가
  아니라 목표 달성).
- **검증**: Unity 에디터 미접속 — 정적으로 UXML 태그 balance(에디터 스크립트로 확인 가능한
  형식적 오류) 확인 완료, 실제 렌더링 순서/스크롤/대륙 아코디언 동작은 QA_Checklist.md에 항목
  추가 후 플레이테스트 필요(아래 "수동 테스트 절차" 참고).
- **변경 파일**: `Assets/UI/CountryStatusPanel.uxml`(섹션 8개 순서 재배치),
  `Assets/Scripts/UI/CountryStatusPanelController.cs`(클래스 상단 XML 문서 주석만 갱신, 실행
  로직 무변경). `CountryStatusPanel.uss`는 무변경.

## Step 89 — CountryStatusPanel v5 Hero Stats 추가 (정체성 확정 완결: "성과 대시보드")

- **배경**: Step 88에서 확정한 "성과 대시보드" 정체성을 화면 최상단에서 즉시 체감할 수 있도록
  Hero Stats(2x2 KPI)를 추가했다(2026-07-15, 사용자 분석안 승인 — 분석 단계 종료, 코드 수정만
  진행). 플레이어가 이 화면을 열었을 때 "내가 얼마나 퍼뜨렸는가/죽였는가/승리까지 얼마나
  남았는가"를 GLOBAL STATUS 배너 바로 다음, WORLD OVERVIEW보다 먼저 보게 만드는 것이 목표.
  비주얼 리디자인/색상 시스템 변경/신규 데이터 모델/신규 계산식은 이번 범위에서 전부 금지 —
  기존 `WorldState`/`Country` 필드와 계산식만 재사용했다.
- **Hero Stats 4항목**: INFECTED(`WorldState.infectedCount`) · DEATHS(`WorldState.deadCount`) ·
  CURE(`WorldState.cureProgress`, `HudController`와 동일한 `×100:F1%` 표기) · EXTINCT(신규
  `ExtinctCountryCount()` 헬퍼 — 기존 `PopulateInfectionSummary()`의 "소멸 국가" 집계식
  `countries.Count(c => c.GetCollapseStage() == CountryCollapseStage.Extinct)`를 그대로 추출해
  Hero Stats와 WORLD OVERVIEW 양쪽이 같은 단일 소스를 읽도록 함). 캡션 없음(요구사항대로
  GLOBAL STATUS → Hero Stats가 바로 이어지게).
- **WORLD OVERVIEW 중복 제거**: `world-summary-rows`의 "총 감염자"/"총 사망자" 행과
  `infection-summary-rows`의 "무감염 국가 수" 행이 Hero Stats(INFECTED/DEATHS/EXTINCT)와
  중복돼 `PopulateWorldSummary()`/`PopulateInfectionSummary()`에서 해당 `AddDataRow()` 호출만
  제거했다. 유지: 세계 인구·세계 감염률·치사율(추정)·경과 일수(world-summary-rows), 감염 국가·
  소멸 국가(infection-summary-rows), population-bar. "감염 국가 현황"은 별도 섹션으로 분리하지
  않고 기존대로 WORLD OVERVIEW 캡션 안에 유지.
- **국가 상태 분포/의료 시스템 현황 → ADVANCED DIAGNOSTICS 강등**: 삭제하지 않고 기본 접힘
  상태의 신규 접기/펼치기 섹션으로 이동했다. UI는 48개국 목록의 대륙 아코디언과 동일한
  `continent-header`/`continent-body`/`continent-header__arrow`/`continent-header__label`
  클래스를 그대로 재사용(신규 CSS 없음) — 단, 상태 관리는 대륙별 `Dictionary`가 아니라 이
  단일 섹션 전용 `bool _advancedDiagnosticsExpanded` 필드 하나로 분리했다(대륙 아코디언과
  서로 영향 없음). `BucketOf()`/`MedicalLoad()`/`MedicalLoadStatus()` 계산식과 `distribution-row`/
  `stat-chip` UI, `PopulateDistribution()`/`PopulateMedicalDistribution()` 로직은 전혀 변경
  없음 — 표시 위치(캡션 2개 "국가 상태 분포"/"의료 시스템 현황"을 섹션 내부에 복원, `modal-section-caption`
  재사용)만 이동했다.
- **USS**: `.hero-stats`(2x2 그리드, `flex-wrap`+`justify-content: space-between`)/
  `.hero-stat-tile`(폭 48%, `--color-surface-soft` 배경+`--stroke-hairline` 테두리)/
  `.hero-stat-tile__label`(`--font-size-xs`)/`.hero-stat-tile__value`(`--font-size-xl`=24px,
  bold) 4개 클래스만 신규 추가. 색상은 새 토큰을 만들지 않고 `Tactical.uss`의 기존
  `data-value--infected`/`data-value--dead`/`data-value--info` modifier(색상만 지정)를
  `hero-stat-tile__value`에 함께 얹어 재사용했다.
- **Controller**: `OnEnable()`에 Hero Label 4개(`hero-infected-value`/`hero-deaths-value`/
  `hero-cure-value`/`hero-extinct-value`) 쿼리 + ADVANCED DIAGNOSTICS 헤더/화살표/바디 쿼리와
  클릭 콜백(`ToggleAdvancedDiagnostics()`) 추가. `HandleWorldStateChanged()`에 `PopulateHeroStats(state,
  countries)` 호출 추가(틱당 1회, 다른 세계 집계와 동일한 갱신 빈도). 신규 메서드:
  `PopulateHeroStats()`, `ExtinctCountryCount()`(static 헬퍼), `ToggleAdvancedDiagnostics()`.
  기존 이벤트 구독 구조(`OnCountryChanged`/`OnWorldStateChanged`)·바인딩 방식(이름 기반
  `root.Q<T>()`)은 전혀 변경 없음.
- **CountryStatusPanel 정체성 최종 기록**: "전략 콘솔"이 아니라 **"성과 대시보드"** — Step 88에서
  확정한 정체성을 이번 Hero Stats 추가로 화면 최상단에 실제로 구현 완료.
- **검증**: Unity 에디터 미접속 — 정적 코드/마크업 검토만 완료. 실제 렌더링(2x2 그리드 정렬,
  ADVANCED DIAGNOSTICS 펼침/접힘 토글, Hero Stats 값 갱신)은 QA_Checklist.md 항목 추가 후
  플레이테스트 필요(아래 "수동 테스트 절차" 참고).
- **변경 파일**: `Assets/UI/CountryStatusPanel.uxml`(Hero Stats 블록 신규, WORLD OVERVIEW 중복
  행 정리는 Controller 쪽에서 처리, 국가 상태 분포/의료 시스템 현황을 ADVANCED DIAGNOSTICS
  접기 섹션으로 이동), `Assets/UI/CountryStatusPanel.uss`(Hero Stats 전용 클래스 4개 신규),
  `Assets/Scripts/UI/CountryStatusPanelController.cs`(Hero Stats 필드/쿼리/Populate 메서드,
  ExtinctCountryCount 헬퍼, ADVANCED DIAGNOSTICS 토글 필드/메서드 신규, 기존 두 Populate
  메서드의 중복 행 제거).

## DESIGN.md v1→v2 개정 이력 (Commit 5, 부록 A 이관)

DESIGN.md는 원래 "철학 설명 위주"였던 v1에서 "수치/표/계약 위주" 구현 명세서인 v2로 전면
개정됐다. Commit 5(DESIGN.md 최신화)에서 이 개정 요약을 부록 A에서 제거하며 이관한다 —
CLAUDE.md 규칙상 "설계 변경 이유"는 DevLog에 남기고 DESIGN.md 본문은 현재 상태만 서술해야
하기 때문(부록 A는 이력이라 본문 자격이 없었음).
...
| Source of Truth 목록 | 4개 파일만 | 11개 파일 전부(Component Registry) |
| Bottom Sheet | 개념 없음, 화면마다 임의 % | Compact/Extended 2변형, 정확한 anchoring 표 |
| Modal | 문서화 안 됨 | `TacticalModalController` 계약 명문화 |
| Badge System | 없음 | CountryPopup `badge-tag` 기준 정식 컴포넌트화 |
| UI Density Modes(3분류) | 있음, Country Dock 등 최신성 저하 | 폐기 — Layout System(4분류)으로 대체 |
| Do/Don't | 일반론 | 실제 발생한 실패 사례 기반 |

## Direction C(Data Wall Terminal) 진행 이력 (Commit 5, 부록 B 이관)

Direction C는 CountryStatusPanel/MainMenu/CountrySelect에 걸쳐 진행된 시각 리디자인이다.
아래 라운드는 전부 DESIGN.md 본문(§2.7/§10/§13/§16/§21 각 절)에 이미 반영 완료된 상태라
부록에 남겨둘 필요가 없어졌다 — Commit 5에서 부록 B를 미확정 항목 2건만 남기고 정리하며
이 진행 이력을 이관한다.

| 라운드 | 화면 | 변경 내용 | 상태 |
|---|---|---|---|
| Color Pass 1 | CountryStatusPanel | hero-stat-tile/status-row/continent-header 배경에 `--color-surface-mint`(신규 토큰) 적용 | 폐기됨 — Color Pass 2가 배경 자체를 없애며 대체, 토큰은 미사용 상태로 잔존 |
| Color Pass 2 | CountryStatusPanel/MainMenu/CountrySelect | hero-stat-tile/status-row/continent-header/pathogen-card/detail-panel/country-row 배경 완전 제거, `--color-bg-root` 0.97→1(지도 유출 방지), country-row 테두리 회색→`--color-accent-glow-soft` | 본문 반영 완료 |
| 밀도 개선(Tier A) | CountryStatusPanel | status-row padding/margin 축소(토큰화), badge-tag에 `--font-size-caption-2xs`+bold 최초 적용 | 본문 반영 완료 |
| 정보 밀도 | MainMenu | `pathogen-card__stat-value`(막대 옆 % 표시) 신규 | 본문 반영 완료 |
| Color Pass 4 | MainMenu | `pathogen-card__stat-fill--severity/lethality/resistance`가 severity 4색("국가 데이터 전용")을 병원체 스탯에 재사용 | 미확정 — 정책 결정 대기(부록 B 잔존 항목으로 계속 추적) |
| Color Pass 5A | CountryStatusPanel | status-row 하단 1px hairline 추가, 대륙 아코디언 좌측바 soft→full 승격, 배지에 `ApplySeverityClass()` 적용 | 본문 반영 완료 |
| Phase 5B | CountryStatusPanel | 감염률/사망률 `data-row` 2줄 → `status-row__stats-summary` 1줄 요약 | 본문 반영 완료 |

## Commit 1~5 — UpgradeTree 캔버스 승격 + ResearchPopup 개편 + 문서 정리

**배경**: UpgradeTree가 기존 branch-board(브랜치 현황판) + research-row(연구 항목 리스트) +
detail-panel(선택 브랜치 요약) 3단 구조로는 "이 연구가 어디로 이어지는지"를 보여줄 수
없다는 문제의식에서, Painter2D 기반 절대좌표 트리 캔버스로 전면 교체하는 작업을 5개 커밋으로
나눠 진행했다(2026-07-17). 각 커밋은 컴파일 가능한 안전한 중간 상태를 유지하며 순차 진행 —
설계 확정 라운드(경로/합류/최종 진화 IA, ResearchPopup Round 4 정보 위계)는 코드 작성 없이
분석만 먼저 거쳤다.

- **Commit 1(캔버스 승격)**: `TreePathElement`(신규, `Painter2D` 기반 연결선 — 이 프로젝트 최초의
  Painter2D 사용 사례) + `UpgradeTreeView.BuildTreeCanvas()`가 `node-scroll.contentContainer`를
  직접 채우도록 전환. 스파이크 단계에서는 `debugShowTreeCanvas` 인스펙터 토글로 기존
  리스트 UI와 분리된 오버레이로 검증했다가, 정상 동작 확인 후 오버레이 구조를 걷어내고
  정식 콘텐츠로 승격했다.
- **Commit 2(branch-board 제거)**: `_branchBoardRows`/`BuildBranchBoard()`/`CreateBranchRow()`
  및 관련 UXML/USS 삭제. 캔버스가 경로/합류/진행 상태를 이미 시각적으로 보여주므로 브랜치
  현황판은 완전히 중복이었다.
- **Commit 3A(research-row 리스트 제거)**: `BuildResearchList()`/`CreateResearchRow()`/
  `AssignCodes()`/`StateCaption()`/`CostText()` 삭제. LAB 캡션 갱신 로직만 `BuildTreeCanvas()`로
  이관.
- **Commit 3B(detail-panel 제거)**: `BuildBranchSummary()`/`AddDetailRow()`/`_selectedBranch`/
  `_branches`/`ResearchBranch`/`BranchOrder`/`_buyButton`(UXML `buy-button` 포함, 오래전부터
  죽어있던 버튼) 삭제. `CategoryClass()`/`CategoryPrefix()`도 호출부 0건 확인 후 함께 제거.
  이 시점부터 UpgradeTree 화면 안에는 캔버스만 남았다.
- **Commit 4(ResearchPopup 개편)**: 노드명 → DNA 비용 → 효과 → 주의 → 연구하기 버튼 → 고급
  정보(Foldout, 기본 접힘) 순서로 정보 위계 재구성. 비용은 `UpgradeManager.GetEffectiveCost()`,
  효과는 `node.effects` 중 0이 아닌 항목만, 주의는 severity/lethality **증가**시에만("발각 위험
  증가"/"치사율 상승" 텍스트 태그, severity 4색 재사용 금지 원칙 준수) 표시하도록 구현. 버튼은
  이미 해금/선행조건 미충족/DNA 부족/구매 가능 4갈래로 텍스트·활성화 분기. 고급 정보는
  `WorldDataManager.CurrentPathogen`(실제 병원체 데이터, 데모 기준값 아님)을 기준으로 감염성/
  심각성/치사율/약물저항 4종의 현재값 vs 적용 후 값을 막대로 비교. 구 "브랜치" `AddRow()` 호출
  제거 — `Show()` 시그니처는 호출부(UpgradeTreeView/UIManager) 무변경을 위해 그대로 유지.
- **Commit 5(문서 정리)**: DESIGN.md §17(UpgradeTree Rules) 전면 재작성, 죽은 컴포넌트 서술
  (§21.1 Branch Board 등) 제거, 신규 컴포넌트(Tree Node/Connector Line/ResearchPopup 상세
  컴포넌트) 문서화, CLAUDE.md TODO 정리(해소된 항목 삭제), `unity-editor-task.md`에 최종
  실기기 검증 섹션 추가. 부록 A/B는 이 섹션과 위 2개 섹션으로 이관.
- **발견된 죽은 코드(문서 정리 중 확인, 코드 삭제는 이번 범위 밖)**: `--color-node-transmission/
  symptom/ability`(§2.3, 좌측 accent-bar 소비처였던 research-row 삭제로 소비처 0),
