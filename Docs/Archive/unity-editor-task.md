...
## 13. UpgradeTree Architecture Refactor — 중복 UIDocument 정리 (2026-07-22)

`UpgradeTreeView.cs`/`UIManager.cs`를 코드 레벨에서 리팩터링해, CountryStatusPanel/RankingPanel과
동일하게 UIDocument 1개 + 컨트롤러 1개가 카테고리(전파/증상/적응) 전환까지 전부 내부에서 처리하도록
바꿨다(예전엔 카테고리별로 별도 UIDocument 3개 — TransmissionTreeUI/SymptomTreeUI/AbilityTreeUI —
를 Show/Hide로 바꿔치기했음). `UIManager.upgradeTreeView` 필드는 `[FormerlySerializedAs
("transmissionTreeView")]`를 붙여뒀으므로 **기존 인스펙터 배선(TransmissionTreeUI를 가리키던 값)이
자동으로 새 필드에 이어진다** — 이 자체는 에디터에서 씬을 열고 저장만 해도 정상 동작하지만,
SymptomTreeUI/AbilityTreeUI GameObject 2개는 이제 아무도 참조하지 않는 죽은 오브젝트로 씬에
남는다(당장 화면을 가리거나 클릭을 막지는 않음 — 각자 `OnEnable()`에서 스스로 `Hide()`를 호출해
`display:None` 상태로 남아있을 뿐이다). 아래는 완전한 아키텍처 정리를 위해 에디터에서 수동으로
해야 하는 작업이다.

- [ ] GamePlay 씬을 열고 `UIManager` GameObject의 Inspector에서 "Upgrade Tree View" 슬롯이
      **TransmissionTreeUI**를 가리키고 있는지 확인(`FormerlySerializedAs` 마이그레이션이
      정상 동작했다면 자동으로 채워져 있어야 함 — 비어있으면 수동으로 TransmissionTreeUI를
      드래그해서 채울 것).
- [ ] Hierarchy에서 **`SymptomTreeUI`**, **`AbilityTreeUI`** GameObject를 삭제한다(둘 다 더 이상
      아무 스크립트도 참조하지 않음 — `UIManager`/`UpgradeTreeView.cs` 어디에서도 이름으로 찾지
      않는다).
- [ ] (선택) 남은 `TransmissionTreeUI` GameObject를 `UpgradeTreeUI` 등으로 이름을 바꿔 "카테고리
      하나 전용"이라는 예전 의미가 더 이상 남지 않게 정리 — 기능에는 영향 없는 순수 정리.
- [ ] Play Mode에서 HUD "업그레이드" 버튼 → 탭(전파/증상/적응) 전환 → 국가현황/랭킹으로 이동 후
      다시 업그레이드로 복귀 시 마지막으로 보던 탭이 그대로 유지되는지 확인(예전엔
      `UIManager._currentUpgradePageIndex`가 담당하던 역할을 이제 `UpgradeTreeView.category`
      필드 하나가 그대로 유지한다 — 씬이 살아있는 동안, 즉 재시작 전까지는 항상 유지됨).
- [ ] Console에 `UpgradeTreeView`/`UIManager` 관련 NullReferenceException/MissingComponentException이
      없는지 확인.
- [x] **BottomHUD 침범 근본 원인 발견 및 수정 (2026-07-22)**: `SafeAreaApplier`가 `upgrade-root`의
      `padding-bottom`을 매 프레임 기기 Safe Area 인셋(보통 82px보다 훨씬 작음)으로 인라인 덮어써
      USS의 `--hud-bottom-nav-height`(82px)가 무력화되고 있었다 — `status-root`/`ranking-root`엔
      애초에 `SafeAreaApplier`가 없어서 이 문제가 없었다. `TransmissionTreeUI`(현재 활성 UpgradeTree
      UIDocument)에서 `SafeAreaApplier` 컴포넌트를 GamePlay.unity에서 직접 제거함(코드 도구로 처리,
      씬 YAML 직접 편집). `SymptomTreeUI`/`AbilityTreeUI`에는 아직 같은 컴포넌트가 남아있지만 두
      GameObject 모두 위 §13 삭제 대상이라 통째로 삭제되면 같이 사라짐 — 별도 제거 불필요.
- [ ] **주의**: `upgrade-root`가 이제 Safe Area(노치/펀치홀/제스처 바) 보정을 전혀 받지 않는다
      (CountryStatusPanel/RankingPanel과 동일한 상태로 맞춘 것이므로 의도된 결과) — 실기기에서
      노치가 있는 기기로 확인할 때, 상단 텍스트가 노치에 가려지는 회귀가 없는지 함께 확인할 것.
      (근본적으로는 `SafeAreaApplier`가 padding을 "대체"가 아니라 "USS 값과 병합/최댓값"으로
      적용하도록 고치는 게 맞지만, 이번 세션에서는 요청 범위인 "CountryStatusPanel과 동일한 구조로
      맞춘다"만 처리함 — SafeAreaApplier 자체의 병합 로직 개선은 별도 작업으로 고려.)
