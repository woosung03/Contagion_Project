...
- [ ] 5분 프로토타입 밸런스 실측 검증 (근거: DevLog Step 93, Step 94) — `cureProgressCoefficient`를
      0.0013 → 0.000975로 재조정(2026-07-24, 인도 발원 방치 플레이 실측 180초 기준 비례 보정).
      아직 재검증 전.
  - [x] 1차 실측(Step 94 조정 전, coefficient=0.0013): 인도 발원 방치 플레이 2회 모두 약 3분
        (180초) — 목표(240초 ±15초) 대비 약 25% 빠름. 이 결과를 근거로 계수 재조정함.
  - [ ] 2차 실측(coefficient=0.000975 반영 후) — 동일 조건(인도 발원, 방치, 최소 3회 반복)으로
        재측정:
    - [ ] Game Over 시간(회차별) 및 평균
    - [ ] 치료제 25% / 50% / 75% / 100% 도달 시점(회차별)
    - [ ] 평균이 240초 ±15초(225~255초) 안에 드는지 확인
    - [ ] 벗어나면 DevLog Step 94의 "검증 안 됨" 항목 참고해 재조정(단순 비례 보정의 한계 —
          2점 보간으로 전환)
  - [ ] 방치 플레이 목표 달성 후: 일반적인 플레이(DNA 수집 + 업그레이드 구매) 기준 5분 내외로
        승패가 결정되는지 별도 검증
  - [ ] 감염 시작 → DNA 획득 → 업그레이드 선택 → 국가 대응 시작 → 감염 확산 가속 → 치료제 개발
        압박 → 승리/패배 전체 루프가 한 판 안에서 자연스럽게 발생하는지 확인
  - [ ] 다른 발원국/다른 병원체 조합도 추가로 확인 권장(국가별 HealthLevel 편차가 큼, DevLog
        Step 94 참고)
