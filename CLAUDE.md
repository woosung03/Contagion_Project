...
4. `Docs/UI_Design.md` — **UI/UXML/USS 작업 시에만.** Design Tokens·Tactical Design System·
   화면별 적용 우선순위.
5. `Docs/Architecture.md` — **새 스크립트/UI 에셋 추가나 코드 위치 확인 시에만.** 네임스페이스·
   폴더 구조·엔진 정보.
6. `Docs/QA_Checklist.md` — **플레이테스트/기능 확인 시.** "이게 잘 되는지 확인해야 함" 항목.
7. `Docs/Release_Checklist.md` — **배포 준비 작업 시.** 저작권/라이선스/스토어 등록/SDK 최종 검증.
8. `Docs/unity-editor-task.md` — **Unity 에디터 수동 작업 확인 시.** 프리팹 연결·씬 배선·
   UIDocument 연결 등 코드 도구로 할 수 없는 작업 목록.
9. 필요 시 `C:\Game\codebase` (참조 전용 코딩 지식 위키, 별도 저장소, **수정 금지**) — Unity UI
...
최근 작업 이력(Step 단위)은 `Docs/DevLog.md` 참고 — 가장 최근은 Step 62(UpgradeTree Tactical
Display 전환, `Docs/UI_Design.md` 11절 구현).
...
**UI 디자인 시스템 적용** (근거: `Docs/UI_Design.md`, 설계안만 완료·실제 화면 반영은 미착수.
남은 5개 화면 우선순위 재평가는 19절 — `CountrySelect > MainMenu > EndingScreen > Tactical
Modal > RankingPanel`)

- CountrySelect에 tactical-panel/data-row 클래스 적용 (`Docs/UI_Design.md` 13절, 우선순위 1)
- MainMenu pathogen-card/detail-panel 전환 (`Docs/UI_Design.md` 12절, 우선순위 2)
...
- **CLAUDE.md 역할은 두 가지뿐이다**: (1) 현재 프로젝트 상태, (2) 다음에 해야 할 작업. 새 정보를
  ...
  | 디자인 토큰, 색상 체계, Typography, Spacing, 공통 UI 컴포넌트, Tactical Design System | `Docs/UI_Design.md` |
