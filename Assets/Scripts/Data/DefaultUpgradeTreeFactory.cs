...
namespace Contagion.Data
{
    ...
    /// 카테고리별로 x 구간을 나누고(감염경로 40~460 / 증상 520~940 / 능력 1000~1420),
    /// y는 선행조건 깊이(티어)에 따라 40씩 증가하는 4단계(0/150/260/370)로 배치했다.
    ///
    ...
    public static class DefaultUpgradeTreeFactory
    {
        public static List<UpgradeNode> BuildDefaultDetailedTree()
        {
            ...
            // ================= 증상 (Symptom) — x: 520~940 =================
            nodes.Add(Node("sym_cough", UpgradeCategory.Symptom, 2, 520, 40, null,
                ("severity", 0.03f)));
            nodes.Add(Node("sym_rash", UpgradeCategory.Symptom, 2, 700, 40, null,
                ("severity", 0.04f)));
            nodes.Add(Node("sym_nausea", UpgradeCategory.Symptom, 2, 880, 40, null,
                ("severity", 0.03f)));

            nodes.Add(Node("sym_fever", UpgradeCategory.Symptom, 3, 520, 150, new[] { "sym_cough" },
                ("severity", 0.05f), ("lethality", 0.01f)));
            nodes.Add(Node("sym_lesion", UpgradeCategory.Symptom, 3, 700, 150, new[] { "sym_rash" },
                ("severity", 0.05f)));
            nodes.Add(Node("sym_vomit", UpgradeCategory.Symptom, 3, 880, 150, new[] { "sym_nausea" },
                ("severity", 0.05f)));

            nodes.Add(Node("sym_pneumonia", UpgradeCategory.Symptom, 5, 520, 260, new[] { "sym_fever" },
                ("severity", 0.06f), ("lethality", 0.03f)));
            nodes.Add(Node("sym_hemorrhage", UpgradeCategory.Symptom, 5, 880, 260, new[] { "sym_vomit" },
                ("severity", 0.06f), ("lethality", 0.03f)));

            nodes.Add(Node("sym_organfailure", UpgradeCategory.Symptom, 12, 700, 370,
                new[] { "sym_pneumonia", "sym_hemorrhage", "sym_lesion" },
                ("lethality", 0.15f), ("severity", 0.05f)));

            // ================= 능력 (Ability) — x: 1000~1420 =================
            nodes.Add(Node("abl_mutation1", UpgradeCategory.Ability, 2, 1000, 40, null,
                ("drugResistance", 0.04f)));
            nodes.Add(Node("abl_stealth1", UpgradeCategory.Ability, 2, 1180, 40, null,
                ("severity", -0.03f)));
            nodes.Add(Node("abl_hardening1", UpgradeCategory.Ability, 2, 1360, 40, null,
                ("drugResistance", 0.05f)));

            nodes.Add(Node("abl_mutation2", UpgradeCategory.Ability, 4, 1000, 150, new[] { "abl_mutation1" },
                ("drugResistance", 0.06f)));
            nodes.Add(Node("abl_stealth2", UpgradeCategory.Ability, 4, 1180, 150, new[] { "abl_stealth1" },
                ("severity", -0.05f)));
            nodes.Add(Node("abl_hardening2", UpgradeCategory.Ability, 4, 1360, 150, new[] { "abl_hardening1" },
                ("drugResistance", 0.07f)));

            nodes.Add(Node("abl_resist1", UpgradeCategory.Ability, 5, 1000, 260, new[] { "abl_mutation2" },
                ("drugResistance", 0.08f)));
            nodes.Add(Node("abl_resist2", UpgradeCategory.Ability, 5, 1360, 260, new[] { "abl_hardening2" },
                ("drugResistance", 0.08f)));

            nodes.Add(Node("abl_finalevo", UpgradeCategory.Ability, 15, 1180, 370,
